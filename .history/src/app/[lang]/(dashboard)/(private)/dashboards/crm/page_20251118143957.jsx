...
// Impor fungsi data langsung dari lib
// PERBAIKAN: Impor 'getRecentBoards' sebagai ganti (atau tambahan) 'getBoardsForWorkspace'
import { ggetBoardsForWorkspace } from '@/libs/data'
...
const CrmDashboardPage = async () => {
  ...
  // PERBAIKAN: Panggil fungsi yang tepat untuk 'Recently visited'
  // Fungsi ini idealnya mengambil data berdasarkan session.user.id
  // dan diurutkan berdasarkan 'lastViewedAt' atau 'updatedAt' di sisi server/database.
  const recentBoards = await getRecentBoards(session?.user?.id)
