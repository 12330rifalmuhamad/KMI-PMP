...
export async function POST(request, { params }) {
  ...
  try {
    ...
    // Validate workspace exists, jika tidak ada, buat workspace default untuk user ini
    let ws = await prisma.workspace.findUnique({ where: { workspaceId: parsedWorkspaceId } })

    if (!ws) {
      ws = await prisma.workspace.create({ data: { workspaceName: 'My Workspace', txtInsertedBy: session.user.name } })
    }
