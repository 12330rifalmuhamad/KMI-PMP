  ...
  try {
    ...
    if (!workspaceAccess) {
      ...
      if (ws.workspaceMember.length === 0 && ws.txtInsertedBy === session.user.name) {
        console.log('[POST /api/workspaces/[workspaceId]/boards] Adding user as workspace member (creator but not member)')
        ...
        return NextResponse.json({ 
          message: 'Access denied. You do not have access to this workspace. Please ensure you are a member of this workspace.' 
        }, { status: 403 })
      }
    }
