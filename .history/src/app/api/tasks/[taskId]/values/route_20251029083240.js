...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const existingValue = await prisma.taskValue.findFirst({
      where: {
    ...
    if (existingValue) {
      ...
      const updatedValue = await prisma.taskValue.update({
        where: { valueId: existingValue.valueId },
        data: { value: txtValue }
      })
      return NextResponse.json(updatedValue)
    } else {
      // Create new value
      const newValue = await prisma.taskValue.create({
        data: {
          taskId: parseInt(taskId),
          columnId: parseInt(intColumn_ID),
          value: txtValue
