...
// PENTING: Gunakan import ini agar tidak terjadi 'Too many connections'
...
// POST: Create a new board in a workspace (Support Template)
export async function POST(request) {
  ...
  try {
    ...
    if (!workspaceAccess) {
      // Auto-add owner if creating first board in their own workspace
    ...
    // A. Cek apakah user memilih template
    if (templateId) {
      ...
      if (template && Array.isArray(template.structure)) {
        ...
        // Map struktur JSON template ke format database Prisma
        columnsToCreate = template.structure.map(col => ({
          ...
          // Pastikan properti ini ada di schema BoardColumn jika ingin menyimpan opsi
          // Jika tidak ada di schema, hapus baris 'options' ini
          // options: col.options || [],
          calculationType: col.calculationType,
          unit: col.unit,
          txtInsertedBy: session.user.name // Wajib diisi sesuai schema Anda
        }))
      }
    }

    // B. Fallback: Jika tidak ada template atau template invalid, gunakan Default Columns
    ...
    const newBoard = await prisma.board.create({
      data: {
        ...
        // Gunakan variabel columnsToCreate yang sudah diproses di atas
        columns: {
      ...
      include: {
        ...
        columns: true // Return columns agar frontend bisa langsung render
      }
    })
