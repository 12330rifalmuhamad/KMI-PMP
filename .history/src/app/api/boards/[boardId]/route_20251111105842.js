...
// Impor komponen-komponen eksternal
  ...
  const handleCreateColumn = async columnData => {
    try {
      ...
      if (res.ok) {
        ...
        setHeaderColumns(prev => [...prev, created])
        mutate(`/api/boards/${board.boardId}`)
      }
    } catch (e) {
    } finally {
  ...
  return (
    <div className='relative'>
                  ...
                  {headerColumns.map(column => {
                    ...
                    if (column.columnName.toLowerCase() === 'item') {
                      ...
                      if (watchedColumnIds.length > 0) {
                        ...
                        group.items.forEach(item => {
                          let relevantTask = false

                          item.values.forEach(val => {
                            if (watchedColumnIds.includes(val.columnId)) {
                              relevantTask = true

                              if (DONE_LABELS.includes(val.value)) {
                                totalDone++
                              }
                            }
                          })
                          if (relevantTask) totalTasks++
                        })
                        ...
                        summaryContent = (
                          <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', gap: 2, padding: '0 8px' }}>
                            ...
                            >
                              <Box sx={{ width: `${percent}%`, height: '100%', backgroundColor: 'success.main' }} />
                            </Box>
      ...
      >
        ...
        {/* Opsi Settings untuk Kolom Progress */}
