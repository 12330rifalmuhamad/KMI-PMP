...
export async function POST(request) {
  ...
  try {
    ...
    // 1. Validasi Input Dasar (PERBAIKAN DISINI: workspaceId)
    if (!boardName || typeof boardName !== 'string' || !workspaceId) {
    ...
    // Gunakan bitActive untuk memastikan member aktif
    let isMember = ws.workspaceMember.some(m => m.userId === userId && m.bitActive === 1)

    // Auto-add owner jika workspace baru/kosong (opsional)
        ...
        if (Array.isArray(structureData)) {
          columnsToCreate = structureData
            ...
            .map((col, index) => ({
              ...
              // Pastikan options adalah array valid
