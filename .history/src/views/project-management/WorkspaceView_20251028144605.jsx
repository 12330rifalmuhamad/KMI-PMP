...
export default function WorkspaceView({ workspaceId }) {
  ...
  const handleDeleteWorkspace = async () => {
    closeMenu()
    const ok = confirm(`Hapus project/workspace "${workspace.workspaceName}"? Semua board di dalamnya akan ikut terhapus.`)
    if (!ok) return
      ...
      <Box className='px-6 pt-6 pb-4 border-b border-divider'>
        ...
        <div className='flex justify-between items-center mb-4'>
          ...
          <div className='flex gap-2'>
            ...
            <Menu id='workspace-menu' anchorEl={menuAnchorEl} open={Boolean(menuAnchorEl)} onClose={closeMenu} keepMounted>
              <MenuItem onClick={handleDeleteWorkspace} sx={{ color: 'error.main' }}>
                <i className='tabler-trash mr-2' /> Hapus Project
              </MenuItem>
            </Menu>
          </div>
        </div>
