...
const NavItem = ({ href, icon, text, active = false, isBoard = false }) => (
  <li>
    <Link href={href}>
      <div
        className={`flex items-center p-2 text-sm rounded-md transition-colors duration-150 ${isBoard ? 'pl-8' : ''} ${active ? 'text-white' : 'text-gray-300'}`}
      >
        {icon}
...
export default function Sidebar() {
  ...
  return (
    <aside className='w-64 h-screen text-white flex flex-col p-2 border-r border-gray-700'>
      {/* Navigasi Atas */}
                  ...
                  <div key={workspace.workspaceId} className='space-y-1'>
                    ...
                    <div className='group flex items-center justify-between'>
                      <button
                        ...
                        className='flex items-center flex-1 text-left py-1'
                      >
                        <i
