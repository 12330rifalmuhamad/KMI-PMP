...
const SimpleModal = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    <div className='fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm'>
      <div className='bg-white text-black-900 rounded-lg shadow-xl w-96 p-6 animate-in fade-in zoom-in duration-200'>
        <h3 className='text-lg font-bold mb-4'>{title}</h3>
        {children}
        <div className='flex justify-end gap-2 mt-4'>
          <button onClick={onClose} className='px-4 py-2 text-sm text-black-600 hover:bg-black-100 rounded'>
            Cancel
          </button>
        </div>
      </div>
  ...
  <li>
    <Link href={href} title={isCollapsed ? text : ''}>
      <div
        ...
        ${active ? 'bg-black-700 text-white' : 'text-black-300 hover:bg-black-700 hover:text-white'}
...
export default function Sidebar() {
  ...
  return (
    <>
      <aside
        className={`h-screen bg-black-800 text-white flex flex-col border-r border-black-700 transition-all duration-300 ease-in-out relative
        ${isCollapsed ? 'w-20' : 'w-64'}`}
      >
        {/* --- TOMBOL TOGGLE --- */}
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className='absolute -right-3 top-6 bg-black-700 border border-black-600 rounded-full p-1 text-black-300 hover:text-white shadow-md z-50 focus:outline-none'
        >
          <i
        ...
        <hr className='border-black-700 mx-2' />
        ...
        <div className='py-4 overflow-y-auto custom-scrollbar flex-1 px-2'>
          ...
          <div className={`flex items-center mb-2 group ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
            <button
              ...
              className={`flex items-center text-black-300 hover:text-white ${isCollapsed ? 'justify-center w-full' : 'flex-1'}`}
              title='Workspaces'
            >
              {isCollapsed ? (
            ...
            {!isCollapsed && (
              <button
                ...
                className='text-black-300 hover:text-white hover:bg-black-700 p-1 rounded transition-colors flex-shrink-0'
          ...
          {isWorkspacesSectionOpen && (
            <div className='flex flex-col space-y-2 mt-2'>
              {!workspaces && !error && !isCollapsed && <div className='text-black-400 px-2 text-xs'>Loading...</div>}
                ...
                workspaces.map(workspace => {
                  ...
                  return (
                    <div key={workspace.workspaceId} className='space-y-1'>
                      ...
                      <div className='group flex items-center justify-between pr-1 hover:bg-black-700/50 rounded'>
                        <button
                        ...
                        >
                          {!isCollapsed && (
                            <i
                              className={`tabler-chevron-right text-black-400 text-xs transition-transform duration-200 mr-1 ${isWorkspaceOpen ? 'rotate-90' : ''}`}
                            />
                          )}
                          <i className={`tabler-folder text-blue-400 ${isCollapsed ? 'text-xl' : 'text-base'}`} />
                          <span
                            className={`ml-2 text-sm font-medium text-black-300 truncate w-28 ${isCollapsed ? 'hidden' : 'block'}`}
                          >
                        ...
                        {!isCollapsed && (
                          <button
                            ...
                            className='text-black-500 hover:text-white p-1 transition-colors flex-shrink-0'
      ...
      >
        <div className='flex flex-col gap-4'>
          <input
            ...
            className='w-full p-2 border border-black-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500'
            value={newTitle}
