...
// --- Import Komponen CreateBoardDialog Baru ---
// Pastikan path import ini sesuai dengan lokasi file CreateBoardDialog.js Anda
import CreateBoardDialog from '@/components/CreateBoardDialog'

const fetcher = url => fetch(url).then(res => res.json())

// --- SIMPLE MODAL COMPONENT (Hanya untuk Workspace sekarang) ---
...
export default function Sidebar() {
  ...
  const theme = useTheme()
  const router = useRouter() // Untuk redirect setelah create
  ...
  const [newTitle, setNewTitle] = useState('') // Hanya dipakai untuk Workspace sekarang
  const [isSubmitting, setIsSubmitting] = useState(false)

  // --- State Baru untuk CreateBoardDialog ---
  ...
  // Updated: Handler untuk membuka Dialog Create Board yang baru
  ...
  // Updated: Callback ketika board berhasil dibuat via Dialog
  const handleBoardCreated = newBoard => {
    // 1. Refresh data boards
    mutate('/api/boards')

    // 2. Expand workspace tempat board baru berada
    if (targetWorkspaceId) {
      setOpenWorkspaces(prev => ({ ...prev, [targetWorkspaceId]: true }))
    }

    // 3. Redirect ke board baru (Opsional, tapi bagus UX-nya)
  ...
  return (
    <>
      ...
      {/* --- MODAL CREATE BOARD (WITH TEMPLATE) --- */}
      {/* Menggunakan komponen dialog baru yang support template */}
      <CreateBoardDialog
