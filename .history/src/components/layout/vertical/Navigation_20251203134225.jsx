    ...
    >
      ...
      <div className='py-4 overflow-y-auto custom-scrollbar flex-1 px-2'>
        ...
        <div className={`flex items-center mb-2 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
          ...
          >
            {isCollapsed ? (
              // Ikon pengganti header saat collapsed
      ...
      {/* --- FOOTER (Guest Info) --- */}
      <div className={`mt-auto p-4 transition-all duration-300 ${isCollapsed ? 'px-0' : ''}`}>
        {!isCollapsed ? (
          <div className='text-center opacity-100 transition-opacity duration-300'>
            <div className='bg-white rounded-full w-12 h-12 mx-auto flex items-center justify-center mb-3'>
              {/* Logo Monday Sederhana */}
              <i className='tabler-brand-monday text-2xl text-[#5034FF]'></i>
            </div>
            <p className='font-semibold text-sm'>Youre a Guest here</p>
            <Link href='/register'>
              <span className='text-blue-400 hover:underline text-sm font-semibold mt-2 block'>Create account</span>
            </Link>
          </div>
        ) : (
          // Versi Mini Footer (Hanya Icon)
          <div className='flex justify-center opacity-100 transition-opacity duration-300'>
            <div
              className='bg-white rounded-full w-8 h-8 flex items-center justify-center cursor-pointer'
              title='Create account'
            >
              <i className='tabler-star text-blue-600 text-sm'></i>
            </div>
          </div>
        )}
      </div>
