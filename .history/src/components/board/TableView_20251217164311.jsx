...
// --- FIXED TAGS POPOVER WITH CORRECT CHECKBOX BEHAVIOR ---
...
export default function TableView({ board }) {
  const { mutate } = useSWRConfig()
  const boardApiEndpoint = `/api/boards/${board.boardId}` // Key untuk Cache SWR
  ...
  // Sinkronisasi awal (tapi kita akan mempercayai cache SWR jika sudah ada update optimistik)
  useEffect(() => {
  ...
  const handleSaveAsTemplate = async name => {
    try {
      // Kita ambil headerColumns karena state ini memegang URUTAN TERBARU (hasil Drag & Drop)
      const columnsToSave = headerColumns.map((col, index) => ({
        columnName: col.columnName,
        columnType: col.columnType,
        options: col.options || [], // Simpan opsi dropdown/status jika ada
        width: col.width,
        sortOrder: index, // Simpan urutan berdasarkan index array saat ini
  ...
  // 5. GENERAL UPDATE (Values & Title & Timeline) - UPDATED FOR PERSISTENCE
  ...
  const handleSaveAsTemplate = async (name) => {
    try {
      // 1. DEBUGGING: Cek data di console browser (F12)
      console.log("Current headerColumns:", headerColumns);
      console.log("Original board columns:", board?.columns);

      // 2. VALIDASI & FALLBACK
      // Jika headerColumns kosong (masalah library DnD), gunakan data asli dari board.columns
      ...
      // Jika masih kosong, hentikan proses agar tidak menyimpan template kosong
      if (sourceColumns.length === 0) {
        alert("Tidak ada kolom untuk disimpan. Pastikan board memiliki kolom.");
        return;
      }
      ...
      const columnsToSave = sourceColumns.map((col, index) => ({
        ...
        columnType: col.columnType,
        options: col.options || [],
        width: col.width || 150, // Default width jika undefined
        sortOrder: index, // Simpan urutan saat ini (PENTING)
        calculationType: col.calculationType || null,
        unit: col.unit || null
      }));

      console.log("Sending payload:", columnsToSave);
  ...
  // =================================================================
  // MAIN RENDER (Helper for Popover State)
  // =================================================================
  ...
  return (
    <div className='relative'>
      <div className='flex justify-between items-center mb-2'>
        <div className='flex gap-2'>
          ...
          <Button
            ...
            className='!border-divider !text-textSecondary'
          >
            Save as Template
          </Button>
        </div>
                              ...
                              <div
                                onClick={e => {
                                  if (
                                    ...
                                    // Klik pada kolom ini di handle di dalam komponennya (atau di atas)
                                  } else {
      ...
      {/* POPOVERS (Menggunakan getFreshItem agar data tidak stale) */}
