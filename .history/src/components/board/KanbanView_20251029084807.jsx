  ...
  const { kanbanColumns, tasksMap } = useMemo(() => {
    ...
    board.groups.forEach(group => {
      group.items.forEach(item => {
        const rawStatus = item.values.find(v => v.columnId === statusColumn.columnId)?.value || 'Blank'

        const statusMap = {
          'Belum Mulai': 'Blank',
          'Sedang Dikerjakan': 'Working on it',
          Buntu: 'Stuck',
          Selesai: 'Done'
        }
    ...
    // Create kanban columns based on unique status values (matching requested UI)
    const statusOrder = ['Working on it', 'Stuck', 'Done', 'Blank']
    ...
    const uncategorizedColumn = { id: 'Blank', title: 'Blank', taskIds: [] }
