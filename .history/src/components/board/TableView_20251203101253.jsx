...
// =================================================================
// UPDATED PROGRESS CELL
// Menggunakan normalizeId agar update status langsung terdeteksi
// =================================================================
const ProgressCell = ({ item, column }) => {
  const progress = useMemo(() => {
    ...
    watchedColumns.forEach(link => {
      ...
      // FIX: Gunakan normalizeId untuk membandingkan ID
    ...
    return { percent, text: `${percent}%` }
  }, [item, column]) // Dependensi sudah benar
...
export default function TableView({ board }) {
  ...
  // ----------------------------------------------------------------
  // UPDATED: handleUpdateValue (Optimistic Update Local State)
  // ----------------------------------------------------------------
  const handleUpdateValue = async (itemToUpdate, columnToUpdate, newValue) => {
    ...
    // 2. UPDATE LOCAL State (Immediate Render for Progress Bar)
  ...
  // ----------------------------------------------------------------
  // UPDATED: handleSaveProgressSettings (Ensure correct column structure)
  // ----------------------------------------------------------------
  const handleSaveProgressSettings = async statusColumnsData => {
    ...
    // Construct robust watchedStatusColumns object
    ...
    // 2. Mutate Local Header State (Force Re-render of ProgressCell)
    setHeaderColumns(headerColumns.map(col => (col.columnId === columnId ? columnToUpdate : col)))
    ...
    return (
      ...
      >
        ...
        {visibleColumns?.map(column => (
          <th
            key={column.columnId}
            className={`p-0 text-left text-sm font-semibold text-textPrimary whitespace-nowrap border-r border-divider ${isMainHeader ? 'table-column-draggable' : ''}`}
          >
                ...
                <tr className='bg-backgroundPaper border-t border-gray-700'>
                  ...
                  {visibleColumns.map(column => {
                    ...
                    if (column.columnName.toLowerCase() === 'item')
                      ...
                      // Optional: Add global progress summary here
                      summaryContent = (
...
const ValueEditorPopover = ({ anchorEl, onClose, column, board, onValueSelect }) => {
  ...
  useEffect(() => { setLabels(initialOptions) }, [initialOptions])

  const handleLabelChange = (id, newText) => {
    setLabels(prev => prev.map(opt => (opt.id === id ? { ...opt, label: newText } : opt)))
  }
  const handleAddNewLabel = () => {
    setLabels(prev => [...prev, { id: crypto.randomUUID(), label: 'New Label', color: 'bg-gray-400', text: 'text-white' }])
  }
  const handleOpenColorPicker = (event, id) => {
    setColorPickerAnchor(event.currentTarget)
    setEditingLabelId(id)
  ...
  const handleColorSelect = newColor => {
    const newTextColor = newColor.includes('/10') || newColor.includes('gray') ? 'text-gray-400' : 'text-white'
    setLabels(prev => prev.map(opt => (opt.id === editingLabelId ? { ...opt, color: newColor, text: newTextColor } : opt)))
    setColorPickerAnchor(null)
    setEditingLabelId(null)
  }
  const handleSaveLabels = async () => {
    ...
    switch (column.columnType) {
      ...
      case 'NUMBER':
        return (
          <div className='p-2'>
            <TextField
              fullWidth size='small' autoFocus type={column.columnType === 'NUMBER' ? 'number' : 'text'}
              defaultValue={column.currentValue || ''}
              placeholder={column.columnType === 'LINK' ? 'https://example.com' : ''}
              onBlur={e => { onValueSelect(e.target.value); onClose() }}
              onKeyDown={e => { if (e.key === 'Enter') { onValueSelect(e.currentTarget.value); onClose() } }}
            />
          </div>
        )
      case 'STATUS':
        ...
        if (isEditingLabels) {
          return (
            <Box className='p-3 flex flex-col gap-2' sx={{ width: 300 }}>
              <Typography variant='body2' className='font-semibold'>Edit Labels</Typography>
              {labels.map(option => (
                <TextField
                  key={option.id} fullWidth size='small' value={option.label}
                  onChange={e => handleLabelChange(option.id, e.target.value)}
                  InputProps={{
                    startAdornment: (
                      <InputAdornment position='start'>
                        <button type='button' className={`w-5 h-5 rounded cursor-pointer ${option.color} flex-shrink-0`} onClick={e => handleOpenColorPicker(e, option.id)} title='Change color' />
                      </InputAdornment>
                    )
                  }}
                />
              ))}
              <Button fullWidth variant='outlined' size='small' startIcon={<i className='tabler-plus' />} onClick={handleAddNewLabel}>New label</Button>
              <Divider className='!my-2' />
              <div className='flex justify-between'>
                <Button variant='text' size='small' onClick={() => { setIsEditingLabels(false); setLabels(initialOptions) }}>Back</Button>
                <Button variant='contained' size='small' onClick={handleSaveLabels} disabled={isLoading}>{isLoading ? 'Saving...' : 'Apply'}</Button>
              </div>
            </Box>
          )
        }
        return (
          <Box className='p-2 flex flex-col gap-2' sx={{ width: 220 }}>
            {labels.map(option => (
              <Button
                key={option.id} variant={isPriority ? 'outlined' : 'contained'}
                className={`!font-semibold !justify-start !shadow-none ${option.color} ${option.text}`}
                style={isPriority ? { borderColor: option.color.split(' ')[1] } : {}}
                onClick={() => { onValueSelect(option.label); onClose() }}
              >
                {option.label}
              </Button>
            ))}
            <Divider className='!my-2' />
            <Button variant='text' size='small' startIcon={<i className='tabler-pencil' />} onClick={() => setIsEditingLabels(true)} className='!normal-case !text-textSecondary'>Edit Labels</Button>
          </Box>
        )
      case 'PERSON':
        return (
          <List>
            {(board?.boardMember || []).map(member => (
              <ListItemButton key={member.userId} onClick={() => { onValueSelect(member.userId.toString()); onClose() }}>
                <ListItemText primary={member.mUser.userName} />
              </ListItemButton>
            ))}
          </List>
      ...
      case 'DATE':
        return (
          <div className='p-2'>
            <TextField type='date' defaultValue={column.currentValue} onChange={e => { onValueSelect(e.target.value); onClose() }} InputLabelProps={{ shrink: true }} autoFocus />
          </div>
        )
      default: return <div className='p-2'>Tipe kolom ini tidak bisa diedit via popover.</div>
    }
  }

  const handleClose = () => { setIsEditingLabels(false); setLabels(initialOptions); onClose() }
  return (
    <Popover open={open} anchorEl={anchorEl} onClose={handleClose} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }} transformOrigin={{ vertical: 'top', horizontal: 'center' }}>
      {renderEditor()}
      <ColorPalettePopover anchorEl={colorPickerAnchor} onClose={() => setColorPickerAnchor(null)} onColorSelect={handleColorSelect} />
    </Popover>
  )
}
...
const CreateColumnModal = ({ open, onClose, onColumnCreated, boardId, initialType = 'TEXT' }) => {
  const [columnName, setColumnName] = useState('')
  const [columnType, setColumnType] = useState(initialType)
  useEffect(() => { setColumnType(initialType) }, [initialType])
  const handleCreate = async () => {
    if (!columnName.trim()) return
    await onColumnCreated({ txtColumnName: columnName, txtColumnType: columnType })
    setColumnName(''); setColumnType('TEXT')
  }
  return (
    <Modal open={open} onClose={onClose}>
      <Card className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 bg-backgroundPaper p-4'>
...
const FilesPopover = ({ anchorEl, onClose, item, column, onFileUploaded }) => {
  ...
  const files = useMemo(() => {
    ...
    return cellValue.split(',').map(url => {
      try {
        const decodedUrl = decodeURIComponent(url)
        const filename = decodedUrl.split('/').pop().substring(decodedUrl.indexOf('_') + 1)
        return { url, name: filename }
      } catch (e) { return { url, name: 'Invalid file name' } }
    })
  }, [cellValue])
  ...
  const handleFileChange = async event => {
    ...
    try {
      ...
      const newValue = cellValue ? `${cellValue},${result.url}` : result.url
      await onFileUploaded(newValue)
    } catch (error) { console.error('File upload error:', error); alert('Gagal mengunggah file.') } finally { setIsUploading(false); onClose() }
  }
  ...
  return (
    <Popover open={open} anchorEl={anchorEl} onClose={onClose} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }} transformOrigin={{ vertical: 'top', horizontal: 'center' }}>
      <input type='file' ref={fileInputRef} onChange={handleFileChange} style={{ display: 'none' }} />
      <Box sx={{ width: 280, p: 1 }}>
        <List dense>
          {files.length > 0 && (
            <>
              <ListSubheader className='!bg-transparent uppercase font-semibold !text-xs !text-textDisabled'>Uploaded Files ({files.length})</ListSubheader>
              {files.map(file => (
                <ListItemButton key={file.url} component='a' href={file.url} target='_blank' rel='noopener noreferrer' onClick={onClose}>
                  <ListItemIcon><i className='tabler-file-text' /></ListItemIcon>
          ...
          {isUploading ? (
            <ListItemButton disabled><ListItemIcon><CircularProgress size={20} /></ListItemIcon><ListItemText primary='Uploading...' /></ListItemButton>
          ) : menuItems.map(item => (
            <ListItemButton key={item.label} onClick={item.action}><ListItemIcon><i className={item.icon} /></ListItemIcon><ListItemText primary={item.label} /></ListItemButton>
          ))}
          <Divider sx={{ my: 1 }} />
          {externalLinks.map(item => (
            <ListItemButton key={item.label} onClick={item.action}><ListItemIcon><i className={item.icon} /></ListItemIcon><ListItemText primary={item.label} /></ListItemButton>
...
const ProgressColumnSettingsModal = ({ open, onClose, board, column, onSave }) => {
  const [isLoading, setIsLoading] = useState(false)
  const statusColumns = useMemo(() => board.columns.filter(c => c.columnType === 'STATUS' && c.columnId !== column.columnId), [board.columns, column.columnId])
  const findWatched = colId => column?.watchedStatusColumns ? column.watchedStatusColumns.find(link => normalizeId(link.statusColumnId) === normalizeId(colId)) : null
  ...
  const handleWeightChange = (columnId, weight) => {
    setColumnSettings(prev => {
      if (prev[columnId]) return { ...prev, [columnId]: { ...prev[columnId], weight: Math.max(0, Math.min(100, parseInt(weight) || 0)) } }
      return prev
    })
  }
  const handleDistributeEqually = () => {
  ...
  const handleSave = async () => {
    setIsLoading(true)
    try {
      const statusColumnsData = Object.entries(columnSettings).filter(([_, settings]) => settings && settings.selected).map(([statusColumnId, settings]) => ({ statusColumnId: parseInt(statusColumnId), weight: Math.max(0, Math.min(100, parseInt(settings?.weight) || 0)) }))
      await onSave(statusColumnsData)
      onClose()
    } catch (error) { console.error('Error:', error); alert('Failed to save') } finally { setIsLoading(false) }
  }
        ...
        <CardContent className='flex flex-col gap-4'>
          ...
          <FormGroup>
            {statusColumns.map(col => {
              ...
              return (
                <Box key={col.columnId} className='flex items-center justify-between gap-4 py-2'>
                  <FormControlLabel control={<Checkbox checked={isSelected} onChange={() => handleToggle(col.columnId)} />} label={col.columnName} className='flex-1' />
                  {isSelected && <TextField type='number' size='small' value={weight} onChange={e => handleWeightChange(col.columnId, e.target.value)} inputProps={{ min: 0, max: 100, step: 1 }} InputProps={{ endAdornment: <InputAdornment position='end'>%</InputAdornment> }} sx={{ width: 100 }} />}
                </Box>
              )
            })}
          </FormGroup>
          <Divider />
          <Button variant='outlined' onClick={handleDistributeEqually} className='!normal-case'>Distribute weight equally</Button>
          <Button variant='contained' onClick={handleSave} disabled={isLoading}>{isLoading ? 'Saving...' : 'Save'}</Button>
        </CardContent>
      </Card>
    </Modal>
  )
...
const ColumnVisibilityPopover = ({ anchorEl, onClose, allColumns, hiddenIds, onToggle }) => {
  const open = Boolean(anchorEl)
  return (
    <Popover open={open} anchorEl={anchorEl} onClose={onClose} anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }} transformOrigin={{ vertical: 'top', horizontal: 'left' }}>
      <Box sx={{ p: 2, width: 250, maxHeight: 400, overflowY: 'auto' }}>
        <Typography variant='subtitle2' sx={{ mb: 1, fontWeight: 'bold' }}>Show/Hide Columns</Typography>
        <Divider sx={{ mb: 1 }} />
        <FormGroup>
          {allColumns.map(col => {
            const isHidden = hiddenIds.includes(col.columnId)
            return (
              <FormControlLabel key={col.columnId} control={<Checkbox checked={!isHidden} onChange={() => onToggle(col.columnId)} size='small' />} label={<span className='text-sm truncate max-w-[150px] block'>{col.columnName}</span>} />
            )
          })}
        </FormGroup>
      </Box>
...
export default function TableView({ board }) {
  ...
  useEffect(() => { setGroups(board?.groups || []) }, [board])
  ...
  const prevColsRef = useRef(headerColumns)
  useEffect(() => {
    const orderChanged = JSON.stringify(prevColsRef.current.map(c => c.columnId)) !== JSON.stringify(headerColumns.map(c => c.columnId))
    if (orderChanged) {
      const persist = async () => {
        for (let index = 0; index < headerColumns.length; index++) {
          const col = headerColumns[index]
          try { await fetch(`/api/columns/${col.columnId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sortOrder: index + 1 }) }) } catch {}
        }
        mutate(`/api/boards/${board.boardId}`)
      }
      persist()
  ...
  // --- HANDLERS ---
  const handleApiCall = async (promise, revalidate = true) => {
    try { await promise; if (revalidate) mutate(`/api/boards/${board.boardId}`) } catch (error) { console.error('API call failed:', error) }
  }

  const handleCreateTask = groupId => handleApiCall(fetch(`/api/groups/${groupId}/tasks`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ txtTaskTitle: newItemTitle || 'New Item' }) })).then(() => { setNewItemTitle(''); setActiveNewItemInput(null) })
  const handleCreateGroup = () => handleApiCall(fetch(`/api/boards/${board.boardId}/groups`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ txtGroupName: 'New Group' }) }))
  const handleCreateColumn = async columnData => { try { const res = await fetch(`/api/boards/${board.boardId}/columns`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(columnData) }); if (res.ok) mutate(`/api/boards/${board.boardId}`) } catch (e) {} finally { setIsColumnModalOpen(false) } }
  ...
  const handleUpdateValue = async (itemToUpdate, columnToUpdate, newValue) => {
    ...
    let value = item.values.find(v => v.columnId === columnToUpdate.columnId)
    if (value) value.value = sanitizedValue
    else item.values.push({ taskValueId: `temp-${Date.now()}`, taskId: item.taskId, columnId: columnIdNumber, value: sanitizedValue })
    mutate(boardApiUrl, newBoardData, { revalidate: false })
    ...
    setGroups(prevGroups => {
      ...
      if (targetGroup) {
         ...
         if (targetItem) {
            let targetVal = targetItem.values.find(v => v.columnId === columnToUpdate.columnId)
            if (targetVal) targetVal.value = sanitizedValue
            else targetItem.values.push({ taskValueId: `temp-${Date.now()}`, taskId: item.taskId, columnId: columnIdNumber, value: sanitizedValue })
         }
      }
      return newGroups
    })

    setEditingCell(null); setEditingTextValue(null)

    try {
      const response = await fetch(`/api/tasks/${itemToUpdate.taskId}/values`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ intColumn_ID: columnIdNumber, txtValue: sanitizedValue }) })
      if (!response.ok) mutate(boardApiUrl, originalBoardData, { revalidate: false })
      else mutate(boardApiUrl)
    } catch (error) { mutate(boardApiUrl, originalBoardData, { revalidate: false }) }
  }

  const handleUpdateTaskTitle = (taskId, newTitle) => handleApiCall(fetch(`/api/tasks/${taskId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ taskTitle: newTitle }) }), false).then(() => setEditingTaskName(null))
  const handleUpdateGroupName = (groupId, newName) => handleApiCall(fetch(`/api/groups/${groupId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ groupName: newName }) }), false).then(() => setEditingGroupName(null))
  const handleSaveTextValue = () => { if (!editingTextValue) return; const { item, column, currentValue } = editingTextValue; handleUpdateValue(item, column, currentValue) }
  const handleDeleteTask = taskId => handleApiCall(fetch(`/api/tasks/${taskId}`, { method: 'DELETE' })).then(() => setMenuAnchor({ anchorEl: null }))
  const handleDeleteGroup = groupId => handleApiCall(fetch(`/api/groups/${groupId}`, { method: 'DELETE' })).then(() => setMenuAnchor({ anchorEl: null }))

  const handleCellClick = (event, item, column) => {
    if (column.columnName.toLowerCase() === 'item') setSelectedItem(item)
    else if (['TEXT', 'LINK', 'NUMBER'].includes(column.columnType)) {
      const cellValue = (item.values || []).find(val => normalizeId(val.columnId) === normalizeId(column.columnId))
      setEditingTextValue({ taskId: item.taskId, columnId: column.columnId, item: item, column: column, currentValue: cellValue?.value || '' })
    } else if (['STATUS', 'PERSON', 'DATE'].includes(column.columnType)) setEditingCell({ anchorEl: event.currentTarget, item, column })
    else if (column.columnType === 'FILES') setFilesPopover({ anchorEl: event.currentTarget, item: item, column: column })
  }

  // --- COLUMN MENUS ---
  const handleOpenAddColumnMenu = event => setAddColumnMenuAnchor(event.currentTarget)
  const handleCloseAddColumnMenu = () => setAddColumnMenuAnchor(null)
  const handleColumnTypeSelect = type => { handleCloseAddColumnMenu(); setSelectedColumnType(type); setIsColumnModalOpen(true) }
  const openColumnMenu = (event, column) => { setColumnMenuAnchor(event.currentTarget); setActiveColumn(column) }
  const closeColumnMenu = () => { setColumnMenuAnchor(null); setActiveColumn(null) }

  const handleRenameColumn = async () => {
    if (!activeColumn) return
    const newName = prompt('Rename column to:', activeColumn.columnName)
    if (!newName || newName.trim() === '' || newName === activeColumn.columnName) return
    await handleApiCall(fetch(`/api/columns/${activeColumn.columnId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ txtColumnName: newName.trim() }) }))
    setHeaderColumns(cols => cols.map(c => (c.columnId === activeColumn.columnId ? { ...c, columnName: newName } : c)))
    closeColumnMenu()
  }
  const handleDeleteColumn = async () => {
  ...
  const handleDuplicateColumn = async () => {
    if (!activeColumn) return
    await handleApiCall(fetch(`/api/boards/${board.boardId}/columns`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ txtColumnName: `${activeColumn.columnName} (Copy)`, txtColumnType: activeColumn.columnType }) }))
    closeColumnMenu()
  }
  const handleAddColumnToRight = async () => { if (!activeColumn) return; await handleCreateColumn({ txtColumnName: 'New Column', txtColumnType: 'TEXT' }); closeColumnMenu() }
  const handleChangeColumnType = async () => {
    if (!activeColumn) return
    const allowed = ['TEXT', 'STATUS', 'PERSON', 'DATE', 'NUMBER', 'CHECKBOX', 'FILES', 'LINK', 'PROGRESS']
    const next = prompt(`Change column type to (${allowed.join(', ')}):`, activeColumn.columnType)
    if (!next || !allowed.includes(next)) return
    await handleApiCall(fetch(`/api/columns/${activeColumn.columnId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ txtColumnType: next }) }))
    closeColumnMenu()
  }

  // Visibility Handlers
  const handleToggleColumnVisibility = columnId => { setHiddenColumnIds(prev => prev.includes(columnId) ? prev.filter(id => id !== columnId) : [...prev, columnId]) }
  const handleHideColumn = () => { if (activeColumn) { handleToggleColumnVisibility(activeColumn.columnId); closeColumnMenu() } }
  ...
  const calculateSummary = (items, columnId, calcType) => {
    const values = items.map(item => { const cellValue = (item.values || []).find(val => normalizeId(val.columnId) === normalizeId(columnId)); return parseFloat(cellValue?.value) || 0 }).filter(v => v !== 0)
    if (values.length === 0) return 0
    switch (calcType) {
      case 'Sum': return values.reduce((acc, val) => acc + val, 0)
      case 'Average': return values.reduce((acc, val) => acc + val, 0) / values.length
      case 'Median': const sorted = [...values].sort((a, b) => a - b); const mid = Math.floor(sorted.length / 2); return sorted.length % 2 !== 0 ? sorted[mid] : (sorted[mid - 1] + sorted[mid]) / 2
  ...
  const handleOpenCalcMenu = (event, column) => { setCalcMenuAnchor(event.currentTarget); setActiveCalcColumn(column) }
  const handleCloseCalcMenu = () => { setCalcMenuAnchor(null); setActiveCalcColumn(null) }
  const handleSelectCalcSetting = async (type, value) => {
    if (!activeCalcColumn) return
    const columnId = activeCalcColumn.columnId
    const newBoardData = structuredClone(board)
    ...
    if (!columnToUpdate) return
    let payload = {}
    if (type === 'calc') { columnToUpdate.calculationType = value; payload = { txtCalculationType: value } }
    else if (type === 'unit') { columnToUpdate.unit = value; payload = { txtUnit: value } }
    mutate(`/api/boards/${board.boardId}`, newBoardData, { revalidate: false })
    setHeaderColumns(headerColumns.map(col => (col.columnId === columnId ? columnToUpdate : col)))
    handleCloseCalcMenu()
    try { await fetch(`/api/columns/${columnId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }) } catch (error) { mutate(`/api/boards/${board.boardId}`) }
  }
  ...
  const handleSaveProgressSettings = async statusColumnsData => {
    ...
    setProgressSettingsModal({ open: false, column: null })
    try { await fetch(`/api/columns/${columnId}/progress-settings`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ statusColumns: statusColumnsData }) }) } catch (error) { mutate(`/api/boards/${board.boardId}`) }
  }

  const calcOptions = ['Sum', 'Average', 'Median', 'Min', 'Max', 'Count']
  const unitOptions = [{ label: 'None', value: null }, { label: '$', value: '$' }, { label: '€', value: '€' }, { label: '£', value: '£' }, { label: '%', value: '%' }]
  const columnTypes = {
    essentials: [{ key: 'STATUS', label: 'Status', icon: 'tabler-progress-check' }, { key: 'TEXT', label: 'Text', icon: 'tabler-t' }, { key: 'PERSON', label: 'People', icon: 'tabler-users' }, { key: 'DROPDOWN', label: 'Dropdown', icon: 'tabler-chevron-down' }, { key: 'DATE', label: 'Date', icon: 'tabler-calendar' }, { key: 'NUMBER', label: 'Numbers', icon: 'tabler-hash' }],
    superUseful: [{ key: 'FILES', label: 'Files', icon: 'tabler-file' }, { key: 'LINK', label: 'Link', icon: 'tabler-link' }, { key: 'PROGRESS', label: 'Progress', icon: 'tabler-progress' }, { key: 'CONNECT', label: 'Connect...', icon: 'tabler-link' }, { key: 'DOC', label: 'monday Doc', icon: 'tabler-file-text' }, { key: 'TIMELINE', label: 'Timeline', icon: 'tabler-timeline' }, { key: 'CHECKBOX', label: 'Checkbox', icon: 'tabler-checkbox' }, { key: 'FORMULA', label: 'Formula', icon: 'tabler-variable' }]
  }
  ...
  const renderHeaderRow = (isMainHeader = false) => {
    return (
      <tr ref={isMainHeader ? headerRef : null} className={`bg-backgroundPaper border-b border-divider ${!isMainHeader ? 'bg-opacity-50' : ''}`}>
        <th className={`sticky left-0 top-0 z-30 bg-backgroundPaper p-0 w-12 border-r border-divider`}>
          <div className='flex justify-center'>{isMainHeader ? <Checkbox /> : <div className="w-4 h-4" />}</div>
        </th>
        {visibleColumns?.map(column => (
          <th key={column.columnId} className={`p-0 text-left text-sm font-semibold text-textPrimary whitespace-nowrap border-r border-divider sticky top-0 z-20 bg-backgroundPaper ${isMainHeader ? 'table-column-draggable' : ''}`}>
            <div className='flex items-center justify-between px-3 py-3'>
              <span className='truncate flex items-center gap-2'>
                {isMainHeader && <i className='tabler-arrows-move col-handle text-textSecondary cursor-grab' />}
                {column.columnName}
              </span>
              <IconButton size='small' color='secondary' onClick={e => openColumnMenu(e, column)}><i className='tabler-dots-vertical' /></IconButton>
            </div>
          </th>
        ))}
        <th className='w-12 border-r border-divider sticky top-0 z-20 bg-backgroundPaper'>
          {isMainHeader && <IconButton size='small' onClick={handleOpenAddColumnMenu}><i className='tabler-plus text-textSecondary' /></IconButton>}
        </th>
      </tr>
    )
  }
  ...
  return (
    <div className='relative'>
      <div className='flex justify-end mb-2'>
        <Button variant='outlined' size='small' startIcon={<i className='tabler-layout-columns' />} onClick={e => setVisMenuAnchor(e.currentTarget)} className='!text-textSecondary !border-divider'>
          Columns {hiddenColumnIds.length > 0 && `(${headerColumns.length - hiddenColumnIds.length}/${headerColumns.length})`}
        </Button>
      </div>
          ...
          <tbody className='divide-y divide-divider'>
            {groups?.map(group => (
              <React.Fragment key={group.groupId}>
                <tr className='bg-backgroundPaper group'>
                  <td className='sticky left-0 z-10 bg-backgroundPaper p-0 w-12 border-r border-divider'>
                    <IconButton size='small' className='opacity-0 group-hover:opacity-100 transition-opacity' onClick={e => setMenuAnchor({ anchorEl: e.currentTarget, type: 'group', id: group.groupId })}><i className='tabler-dots-vertical' /></IconButton>
                  </td>
                  <td colSpan={visibleColumns.length + 1} className='p-2 font-bold text-textPrimary' style={{ borderLeft: `4px solid ${group.groupColor}` }}>
                    {editingGroupName?.groupId === group.groupId ? (
                      <TextField autoFocus fullWidth variant='standard' value={editingGroupName.currentName} onChange={e => setEditingGroupName({ ...editingGroupName, currentName: e.target.value })} onBlur={() => handleUpdateGroupName(group.groupId, editingGroupName.currentName)} onKeyDown={e => e.key === 'Enter' && handleUpdateGroupName(group.groupId, editingGroupName.currentName)} InputProps={{ disableUnderline: true, className: '!text-textPrimary !font-semibold' }} />
                    ) : (
                      <Typography variant='subtitle2' onClick={() => setEditingGroupName({ groupId: group.groupId, currentName: group.groupName })} className='cursor-pointer'>{group.groupName}</Typography>
                    )}
                  </td>
                </tr>
                ...
                {group.items?.map(item => (
                  <tr key={item.taskId} className='hover:bg-action-hover group'>
                    <td className='sticky left-0 z-10 bg-backgroundPaper p-0 w-12 border-r border-divider'>
                      <div className='flex justify-center items-center h-full'>
                        <Checkbox className='group-hover:hidden' />
                        <IconButton size='small' className='hidden group-hover:block' onClick={e => setMenuAnchor({ anchorEl: e.currentTarget, type: 'task', id: item.taskId })}><i className='tabler-dots-vertical' /></IconButton>
                      </div>
                    </td>
                    {visibleColumns.map(column => {
                      const cellValue = (item.values || []).find(val => normalizeId(val.columnId) === normalizeId(column.columnId))
                      let cellContent
                      const isEditingItemTitle = editingTaskName?.taskId === item.taskId && column.columnName.toLowerCase() === 'item'
                      const isEditingTextValue = editingTextValue?.taskId === item.taskId && editingTextValue?.columnId === column.columnId

                      switch (column.columnType) {
                        case 'PERSON': cellContent = (<div className='flex justify-center'><PersonAvatar user={cellValue ? findUserById(cellValue.value) : null} /></div>); break
                        case 'STATUS': cellContent = <StatusCell value={cellValue?.value} column={column} />; break
                        case 'DATE': cellContent = <DateCell value={cellValue?.value} />; break
                        case 'CHECKBOX': cellContent = (<div className='flex justify-center'><Checkbox checked={cellValue?.value === 'true'} onChange={e => handleUpdateValue(item, column, e.target.checked.toString())} onClick={e => e.stopPropagation()} /></div>); break
                        case 'FILES': cellContent = <FilesCell value={cellValue?.value} />; break
                        case 'LINK': cellContent = <LinkCell value={cellValue?.value} />; break
                        case 'NUMBER': cellContent = <span>{cellValue?.value || '—'}</span>; break
                        case 'PROGRESS': cellContent = <ProgressCell item={item} column={column} />; break
                        case 'TEXT': if (column.columnName.toLowerCase() === 'item') cellContent = item.taskTitle; else cellContent = <span>{cellValue?.value || '—'}</span>; break
                        default: cellContent = <span>{cellValue?.value || '—'}</span>; break
                      }

                      return (
                        <td key={`${item.taskId}-${column.columnId}`} className={`p-0 h-10 border-r border-divider`}>
                          <div
                            onClick={e => { if (isEditingItemTitle || isEditingTextValue) return; handleCellClick(e, item, column) }}
                            className={`flex items-center w-full h-full cursor-pointer ${ column.columnType !== 'STATUS' && column.columnType !== 'TEXT' && column.columnName.toLowerCase() !== 'item' ? 'justify-center' : 'justify-start' } ${column.columnType !== 'STATUS' ? 'px-3' : ''}`}
                          >
                            {isEditingItemTitle ? (
                              <TextField autoFocus fullWidth variant='standard' value={editingTaskName.currentName} onChange={e => setEditingTaskName({ ...editingTaskName, currentName: e.target.value })} onBlur={() => handleUpdateTaskTitle(item.taskId, editingTaskName.currentName)} onKeyDown={e => e.key === 'Enter' && handleUpdateTaskTitle(item.taskId, editingTaskName.currentName)} InputProps={{ disableUnderline: true, className: '!text-textPrimary !font-semibold' }} onClick={e => e.stopPropagation()} />
                            ) : isEditingTextValue ? (
                              <TextField autoFocus fullWidth variant='standard' type={column.columnType === 'NUMBER' ? 'number' : 'text'} value={editingTextValue.currentValue} onChange={e => setEditingTextValue({ ...editingTextValue, currentValue: e.target.value })} onBlur={handleSaveTextValue} onKeyDown={e => { if (e.key === 'Enter') handleSaveTextValue() }} InputProps={{ disableUnderline: true, className: '!text-textPrimary' }} onClick={e => e.stopPropagation()} />
                            ) : column.columnName.toLowerCase() === 'item' ? (
                              <strong onClick={e => { e.stopPropagation(); setEditingTaskName({ taskId: item.taskId, currentName: item.taskTitle }); setSelectedItem(null) }}>{cellContent}</strong>
                            ) : (
                              <div className='w-full h-full'>{cellContent}</div>
                            )}
                          </div>
                ...
                <tr className='bg-backgroundPaper'>
                  <td className='sticky left-0 z-10 bg-backgroundPaper p-0 w-12 border-r border-divider'><div className='flex justify-center'><Checkbox className='opacity-0' /></div></td>
                  <td className='p-2 text-textSecondary border-r border-divider'>
                    {activeNewItemInput === group.groupId ? (
                      <TextField fullWidth autoFocus variant='standard' value={newItemTitle} onChange={e => setNewItemTitle(e.target.value)} onBlur={() => handleCreateTask(group.groupId)} onKeyDown={e => e.key === 'Enter' && handleCreateTask(group.groupId)} placeholder='+ Add Item' InputProps={{ disableUnderline: true }} />
                    ) : (
                      <button onClick={() => setActiveNewItemInput(group.groupId)} className='flex items-center w-full text-left'><i className='tabler-plus mr-2' />+ Add Item</button>
                    )}
                  </td>
                  <td colSpan={visibleColumns.length} className='border-r border-divider'></td>
                </tr>
                ...
                <tr className='bg-backgroundPaper border-t border-gray-700'>
                  <td className='sticky left-0 z-10 bg-backgroundPaper p-0 w-12 border-r border-divider'></td>
                  {visibleColumns.map(column => {
                    let summaryContent = null; const calcType = column.calculationType || 'Sum'; const unit = column.unit || null
                    if (column.columnName.toLowerCase() === 'item') summaryContent = (<Typography variant='caption' className='text-gray-400 px-3'>Count: {group.items?.length || 0}</Typography>)
                    else if (column.columnName === 'Status') {
                      const summary = getStatusSummary(group)
                      summaryContent = (<div className='flex h-4 rounded-sm overflow-hidden mx-2'>{Object.entries(summary).map(([status, count]) => { let bgColor = 'bg-gray-500'; if (status === 'Selesai') bgColor = 'bg-green-500'; else if (status === 'Sedang Dikerjakan') bgColor = 'bg-yellow-500'; else if (status === 'Buntu') bgColor = 'bg-red-500'; return (<div key={status} className={bgColor} style={{ flexGrow: count }} title={`${status}: ${count}`}></div>) })}</div>)
                    } else if (column.columnType === 'NUMBER') {
                      const result = calculateSummary(group.items, column.columnId, calcType)
                      summaryContent = (<Button onClick={e => handleOpenCalcMenu(e, column)} className='!text-textSecondary !normal-case !font-semibold !p-0 !min-w-0'>{formatCalculation(result, unit, calcType)}</Button>)
                    } else if (column.columnType === 'PROGRESS') {
                       // Optional: Add global progress summary here
                       summaryContent = (<Typography variant='caption' className='text-gray-500 px-3'>-</Typography>)
                    }
                    return (<td key={`footer-${column.columnId}`} className='p-2 border-r border-divider text-center'>{summaryContent}</td>)
                  })}
                  <td className='border-r border-divider'></td>
                </tr>
              </React.Fragment>
      ...
      <Button startIcon={<i className='tabler-plus' />} className='!mt-4 !normal-case' onClick={handleCreateGroup}>Add new group</Button>

      <Menu anchorEl={menuAnchor.anchorEl} open={Boolean(menuAnchor.anchorEl)} onClose={() => setMenuAnchor({ anchorEl: null })}>
        <MenuItem onClick={() => menuAnchor.type === 'task' ? handleDeleteTask(menuAnchor.id) : handleDeleteGroup(menuAnchor.id)} sx={{ color: 'error.main' }}>Delete</MenuItem>
      </Menu>

      {selectedItem && <ItemDetailPanel item={selectedItem} board={board} onClose={() => setSelectedItem(null)} />}
      {editingCell && (<ValueEditorPopover anchorEl={editingCell.anchorEl} onClose={() => setEditingCell(null)} column={{ ...editingCell.column, currentValue: (editingCell.item.values || []).find(v => normalizeId(v.columnId) === normalizeId(editingCell.column.columnId))?.value }} board={board} onValueSelect={newValue => handleUpdateValue(editingCell.item, editingCell.column, newValue)} />)}
      <FilesPopover anchorEl={filesPopover.anchorEl} onClose={() => setFilesPopover({ anchorEl: null, item: null, column: null })} item={filesPopover.item} column={filesPopover.column} onFileUploaded={newValue => { handleUpdateValue(filesPopover.item, filesPopover.column, newValue) }} />
      <Menu anchorEl={addColumnMenuAnchor} open={isAddColumnMenuOpen} onClose={handleCloseAddColumnMenu} PaperProps={{ style: { maxHeight: 500, width: '350px' } }}>
        <div className='px-4 pt-2 pb-1'><TextField fullWidth size='small' variant='outlined' placeholder='Search or describe your column' InputProps={{ startAdornment: <i className='tabler-search text-textDisabled mr-2' /> }} /></div>
        <ListSubheader className='!bg-transparent uppercase font-semibold !text-xs !text-textDisabled'>Essentials</ListSubheader>
        <div className='grid grid-cols-2 gap-1 px-2'>{columnTypes.essentials.map(type => (<MenuItem key={type.key} onClick={() => handleColumnTypeSelect(type.key)} className='!rounded-md'><ListItemIcon sx={{ color: 'text.secondary', minWidth: '32px' }}><i className={`${type.icon} text-lg`} /></ListItemIcon><ListItemText primary={type.label} primaryTypographyProps={{ variant: 'body2' }} /></MenuItem>))}</div>
        <ListSubheader className='!bg-transparent uppercase font-semibold !text-xs !text-textDisabled mt-2'>Super useful</ListSubheader>
        <div className='grid grid-cols-2 gap-1 px-2'>{columnTypes.superUseful.map(type => (<MenuItem key={type.key} onClick={() => handleColumnTypeSelect(type.key)} className='!rounded-md'><ListItemIcon sx={{ color: 'text.secondary', minWidth: '32px' }}><i className={`${type.icon} text-lg`} /></ListItemIcon><ListItemText primary={type.label} primaryTypographyProps={{ variant: 'body2' }} /></MenuItem>))}</div>
        <Divider className='!my-2' /><MenuItem onClick={handleCloseAddColumnMenu} className='justify-center'><Typography variant='body2' color='primary'>More columns</Typography></MenuItem>
      </Menu>

      <CreateColumnModal open={isColumnModalOpen} onClose={() => setIsColumnModalOpen(false)} boardId={board.boardId} onColumnCreated={handleCreateColumn} initialType={selectedColumnType} />

      <Menu anchorEl={columnMenuAnchor} open={Boolean(columnMenuAnchor)} onClose={closeColumnMenu} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} transformOrigin={{ vertical: 'top', horizontal: 'right' }}>
        <MenuItem onClick={handleRenameColumn}><ListItemIcon><i className='tabler-pencil text-lg' /></ListItemIcon><ListItemText>Rename</ListItemText></MenuItem>
        <MenuItem onClick={handleHideColumn}><ListItemIcon><i className='tabler-eye-off text-lg' /></ListItemIcon><ListItemText>Hide column</ListItemText></MenuItem>
        {activeColumn?.columnType === 'PROGRESS' && (<MenuItem onClick={() => { setProgressSettingsModal({ open: true, column: activeColumn }); closeColumnMenu() }}><ListItemIcon><i className='tabler-settings text-lg' /></ListItemIcon><ListItemText>Progress Settings</ListItemText></MenuItem>)}
        <Divider className='!my-1' />
        <MenuItem onClick={handleDuplicateColumn}><ListItemIcon><i className='tabler-copy text-lg' /></ListItemIcon><ListItemText>Duplicate column</ListItemText></MenuItem>
        <MenuItem onClick={handleAddColumnToRight}><ListItemIcon><i className='tabler-plus text-lg' /></ListItemIcon><ListItemText>Add column to the right</ListItemText></MenuItem>
        <MenuItem onClick={handleChangeColumnType}><ListItemIcon><i className='tabler-arrows-exchange text-lg' /></ListItemIcon><ListItemText>Change column type</ListItemText></MenuItem>
        <Divider className='!my-1' />
        <MenuItem onClick={handleDeleteColumn} sx={{ color: 'error.main' }}><ListItemIcon><i className='tabler-trash text-lg' /></ListItemIcon><ListItemText>Delete</ListItemText></MenuItem>
      </Menu>

      <Menu anchorEl={calcMenuAnchor} open={Boolean(calcMenuAnchor)} onClose={handleCloseCalcMenu} PaperProps={{ sx: { width: 300 } }}>
        <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Typography variant='caption' className='uppercase font-semibold text-textDisabled'>Unit</Typography>
          <Box sx={{ display: 'flex', border: '1px solid', borderColor: 'divider', borderRadius: 1, overflow: 'hidden' }}>
            {unitOptions.map(opt => { const isSelected = (activeCalcColumn?.unit || null) === opt.value; return (<Button key={opt.label} onClick={() => handleSelectCalcSetting('unit', opt.value)} sx={{ minWidth: 40, borderRadius: 0, backgroundColor: isSelected ? 'primary.main' : 'transparent', color: isSelected ? 'white' : 'text.secondary', borderRight: '1px solid', borderColor: 'divider', '&:last-child': { borderRight: 'none' } }}>{opt.label}</Button>) })}
            <TextField variant='standard' placeholder='Type your own' size='small' sx={{ flex: 1, px: 1, input: { color: 'text.primary', fontSize: '0.875rem' } }} />
          </Box>
          <Typography variant='caption' className='uppercase font-semibold text-textDisabled' sx={{ mt: 1, display: 'block' }}>Calculation</Typography>
          <Box sx={{ display: 'flex', border: '1px solid', borderColor: 'divider', borderRadius: 1, overflow: 'hidden' }}>
            {calcOptions.map(type => { const isSelected = type === (activeCalcColumn?.calculationType || 'Sum'); return (<Button key={type} onClick={() => handleSelectCalcSetting('calc', type)} sx={{ flex: 1, borderRadius: 0, backgroundColor: isSelected ? 'primary.main' : 'transparent', color: isSelected ? 'white' : 'text.secondary', borderRight: '1px solid', borderColor: 'divider', '&:last-child': { borderRight: 'none' }, fontSize: '0.875rem', textTransform: 'capitalize' }}>{type}</Button>) })}
          </Box>
        </Box>
      </Menu>

      {progressSettingsModal.open && <ProgressColumnSettingsModal open={progressSettingsModal.open} onClose={() => setProgressSettingsModal({ open: false, column: null })} board={board} column={progressSettingsModal.column} onSave={handleSaveProgressSettings} />}
      <ColumnVisibilityPopover anchorEl={visMenuAnchor} onClose={() => setVisMenuAnchor(null)} allColumns={headerColumns} hiddenIds={hiddenColumnIds} onToggle={handleToggleColumnVisibility} />
    </div>
  )
}
