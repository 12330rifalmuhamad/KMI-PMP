...
const KanbanList = props => {
  ...
  useEffect(() => {
    if (tasksList !== tasks) {
      // Update the column's task list
      const newColumns = columns.map(col => {
        if (col.id === column.id) {
          return { ...col, taskIds: tasksList.map(task => task.taskId) }
        }
  ...
  useEffect(() => {
    ...
    const newTasksList = tasksList.filter(task => task && taskIds.includes(task.taskId))
