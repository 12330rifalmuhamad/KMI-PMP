  ...
  return (
    <div
      ...
      onDrop={e => {
        ...
        console.log('Drop event - taskId:', taskId, 'target column:', column.id)

        if (taskId && onTaskMove) {
          const sourceColumn = e.dataTransfer.getData('source-column')

          console.log('Source column:', sourceColumn, 'Target column:', column.id)

          if (sourceColumn && sourceColumn !== column.id) {
            console.log(`Dropping task ${taskId} from ${sourceColumn} to ${column.id}`)
            onTaskMove(parseInt(taskId), sourceColumn, column.id)
          } else {
            console.log('Same column or no source column, ignoring drop')
          }
        } else {
          console.log('No taskId or onTaskMove function')
        }
      }}
    >
      {editDisplay ? (
        ...
        <div
          ...
          className={classnames(
            'flex items-center justify-between is-[16.5rem] bs-[2.125rem] mbe-4 flex-shrink-0 rounded-md text-white',
