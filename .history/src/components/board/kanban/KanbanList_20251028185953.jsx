...
const KanbanList = props => {
  ...
  useEffect(() => {
    if (tasksList !== tasks) {
      const nextTaskIds = tasksList.map(task => task.taskId)
      ...
      const newColumns = columns.map(col => {
        ...
        const isSameOrder =
          col.taskIds?.length === nextTaskIds.length &&
          col.taskIds?.every((id, idx) => id === nextTaskIds[idx])
  ...
  useEffect(() => {
    ...
    const desiredIds = colForThis?.taskIds || []
    ...
    const orderedNextList = desiredIds
      .map(id => nextList.find(t => t.taskId === id))
      .filter(Boolean)

    const currentIds = tasksList.map(t => t.taskId)

    const isSame =
      currentIds.length === orderedNextList.length &&
      currentIds.every((id, idx) => id === orderedNextList[idx].taskId)
