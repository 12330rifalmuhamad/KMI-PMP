...
const KanbanList = props => {
  // Props
  const { column, tasks, board, setDrawerOpen, columns, setColumns, currentTask, onOpenDrawer, onTaskMove, mutate } = props
  ...
  const [tasksListRef, tasksList, setTasksList] = useDragAndDrop(tasks, {
    ...
    dropZone: el => !!el?.classList?.contains?.('kanban-column-draggable'),
    onDrop: ({ origin, target }) => {
      if (origin && target && origin !== target) {
        const movedTask = tasksList.find(task => task.taskId === origin)
        if (movedTask && onTaskMove) {
          onTaskMove(movedTask.taskId, column.id, target)
        }
      }
    }
  })
  ...
  useEffect(() => {
    ...
    // Derive next list primarily from source tasks prop; fallback to existing tasksList
    const taskByIdFromProps = new Map((tasks || []).filter(Boolean).map(t => [t.taskId, t]))
    const taskByIdFromState = new Map((tasksList || []).filter(Boolean).map(t => [t.taskId, t]))

    const nextList = desiredIds.map(id => taskByIdFromProps.get(id) || taskByIdFromState.get(id)).filter(Boolean)
  ...
  return (
    <div ref={tasksListRef} className='flex flex-col is-[16.5rem] kanban-column-draggable h-full'>
      {editDisplay ? (
        <form
          className='flex items-center mbe-4 flex-shrink-0'
        ...
        <div
          ...
          className={classnames(
            'flex items-center justify-between is-[16.5rem] bs-[2.125rem] mbe-4 flex-shrink-0',
            styles.kanbanColumn
          )}
        >
