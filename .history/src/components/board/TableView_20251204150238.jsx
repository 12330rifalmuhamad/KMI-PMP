...
const TagsCell = ({ value, column }) => {
  if (!value) return <span className='text-gray-500 text-xs px-2'>—</span>
  const tags = value.split(',')

  const getTagColor = tagName => {
    const opt = column.options?.find(o => o.label === tagName)

    return opt?.color || 'bg-gray-500'
  }

  return (
    <div className='flex flex-wrap gap-1 px-2 py-1 items-center justify-start w-full h-full overflow-hidden'>
      {tags.map((tag, index) => (
        <span
          key={index}
          className={`text-[10px] px-2 py-0.5 rounded-full text-white whitespace-nowrap ${getTagColor(tag)}`}
        >
          #{tag}
        </span>
      ))}
...
// ==========================================================
// PERBAIKAN: FilesCell Tanpa Background Abu-abu
// ==========================================================
const FilesCell = ({ value }) => {
  const fileCount = value ? value.split(',').length : 0

  if (fileCount === 0) return <div className='flex items-center justify-center h-full'>-</div>

  return (
    <div className='flex items-center justify-center h-full gap-1 text-textSecondary'>
      <i className='tabler-paperclip' style={{ fontSize: '1.1rem' }} />
...
// ==========================================================
// PERBAIKAN: LinkCell Tanpa Background Abu-abu
// ==========================================================
const LinkCell = ({ value }) => {
  if (!value) return <div className='flex items-center justify-center h-full text-gray-400'>—</div>
  let href = value
  ...
  return (
    <div className='w-full h-full flex items-center px-3'>
      <a
        ...
        className='text-blue-500 hover:underline truncate w-full block'
...
const ProgressCell = ({ item, column }) => {
  const progress = useMemo(() => {
    ...
    if (watchedColumnIds.length === 0) return { percent: 0, text: 'N/A' }
    ...
    watchedColumnIds.forEach(colId => {
      // Logika bobot
      // Untuk saat ini kita anggap bobot rata jika belum ada data weight spesifik
      // (Perlu update ProgressCell.jsx terpisah untuk full logic bobot)
      const weight = 1
...
// =================================================================
// VALUE EDITOR POPOVER (HANDLES STATUS, PERSON, DATE, TAGS)
// =================================================================
const ValueEditorPopover = ({ anchorEl, onClose, column, board, onValueSelect }) => {
  ...
  const [searchTerm, setSearchTerm] = useState('') // Untuk Tags
  ...
  // --- Logic Tags ---
  const handleToggleTag = async (label, color) => {
    const currentTags = column.currentValue ? column.currentValue.split(',') : []

    const newTags = currentTags.includes(label) ? currentTags.filter(t => t !== label) : [...currentTags, label]

    onValueSelect(newTags.join(','))

    const exists = column.options?.some(opt => opt.label === label)

    if (!exists && color) {
      const newOptions = [...(column.options || []), { label, color }]

      // Optimistic update options
      const boardApiUrl = `/api/boards/${board.boardId}`
      const newBoardData = structuredClone(board)
      const col = newBoardData.columns.find(c => c.columnId === column.columnId)
      ...
      await fetch(`/api/columns/${column.columnId}/options`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ options: newOptions, renameMap: {} })
      })
    }
  }
  ...
  const renderEditor = () => {
    switch (column.columnType) {
      ...
      case 'TAGS':
        ...
        const filteredTags = availableOptions.filter(opt => opt.label.toLowerCase().includes(searchTerm.toLowerCase()))
        ...
        const newTagColor = randomColors[Math.floor(Math.random() * randomColors.length)]
          ...
          <Box sx={{ width: 250, p: 1 }}>
            ...
            <List dense sx={{ maxHeight: 200, overflow: 'auto', mt: 1 }}>
              {filteredTags.map(opt => (
                <ListItemButton key={opt.label} onClick={() => handleToggleTag(opt.label)}>
                  <Checkbox checked={currentTags.includes(opt.label)} size='small' />
                  <div className={`w-3 h-3 rounded-full mr-2 ${opt.color}`}></div>
                  <ListItemText primary={opt.label} />
                </ListItemButton>
              ))}
              {searchTerm && !availableOptions.some(o => o.label.toLowerCase() === searchTerm.toLowerCase()) && (
                <ListItemButton
                  onClick={() => {
                    handleToggleTag(searchTerm, newTagColor)
                    setSearchTerm('')
                  }}
                >
                  <ListItemIcon>
                  ...
                  <ListItemText primary={`Create "${searchTerm}"`} />
                </ListItemButton>
              )}
            </List>
          </Box>
...
const ProgressColumnSettingsModal = ({ open, onClose, board, column, onSave }) => {
  ...
  // State untuk menyimpan { [colId]: { selected: boolean, weight: number } }
  const [columnSettings, setColumnSettings] = useState(() => {
    const initialSettings = {}

    ;(column?.watchedStatusColumns || []).forEach(link => {
      initialSettings[link.statusColumnId] = { selected: true, weight: link.weight || 0 }
    })
  ...
  const handleToggle = columnId => {
    setColumnSettings(prev => {
      ...
      if (!newSettings[columnId]) {
        ...
        newSettings[columnId] = { ...newSettings[columnId], selected: !newSettings[columnId].selected }
      }
  ...
  const handleWeightChange = (columnId, weightStr) => {
    ...
    setColumnSettings(prev => ({
      ...prev,
      [columnId]: { ...(prev[columnId] || { selected: true }), weight: Math.max(0, Math.min(100, weight)) }
    }))
  }
...
export default function TableView({ board }) {
  ...
  const handleCellClick = (event, item, column) => {
    ...
    } else if (['STATUS', 'PERSON', 'DATE', 'TAGS'].includes(column.columnType)) {
  ...
  const handleDeleteColumn = async () => {
    if (!activeColumn) return
    if (!confirm(`Delete column "${activeColumn.columnName}"?`)) return
  ...
  const handleChangeColumnType = async () => {
    if (!activeColumn) return
    const allowed = ['TEXT', 'STATUS', 'PERSON', 'DATE', 'NUMBER', 'CHECKBOX', 'FILES', 'LINK', 'PROGRESS', 'TAGS']
  ...
  const handleSaveProgressSettings = async statusColumnsData => {
    ...
    columnToUpdate.watchedStatusColumns = statusColumnsData.map(({ statusColumnId, weight }) => {
      ...
      return { progressColumnId: columnId, statusColumnId: statusColumnId, weight: weight, statusColumn: statusCol }
    })
    mutate(boardApiUrl, newBoardData, { revalidate: false })
    const newHeaderColumns = headerColumns.map(col => (col.columnId === columnId ? columnToUpdate : col))
  ...
  const columnTypes = {
    ...
    superUseful: [
      ...
      { key: 'TAGS', label: 'Tags', icon: 'tabler-hash' },
                    ...
                    {headerColumns.map(column => {
                      ...
                      switch (column.columnType) {
                        ...
                        case 'PROGRESS':
                          ...
                          break
                        case 'TAGS':
                          cellContent = <TagsCell value={cellValue?.value} column={column} />
                          break
                        case 'TEXT':
