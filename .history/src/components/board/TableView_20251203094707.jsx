...
// =================================================================
// DEFINISI KOMPONEN PROGRESS CELL
// =================================================================
const ProgressCell = ({ item, column }) => {
  const progress = useMemo(() => {
    ...
    // Jika total weight 0, gunakan perhitungan sederhana (equal weight)
    if (totalWeight === 0) {
...
// =================================================================
// DEFINISI KOMPONEN COLOR PALETTE POPOVER
// =================================================================
const colorPalette = [
  'bg-green-500',
  'bg-yellow-500',
...
// =================================================================
// KOMPONEN YANG DIPERBAIKI: ProgressColumnSettingsModal
// =================================================================
const ProgressColumnSettingsModal = ({ open, onClose, board, column, onSave }) => {
  const [isLoading, setIsLoading] = useState(false)

  // 1. Definisikan statusColumns di luar state agar bisa diakses di semua hook
  const statusColumns = useMemo(() => {
    return board.columns.filter(c => c.columnType === 'STATUS' && c.columnId !== column.columnId)
  }, [board.columns, column.columnId])

  // Helper untuk mencari data kolom yang sudah diawasi
  const findWatched = colId => {
    if (!column || !column.watchedStatusColumns) return null

    return column.watchedStatusColumns.find(link => link.statusColumnId === colId)
  }

  // 2. PERBAIKI: Inisialisasi useState dengan SEMUA kolom status
  const [columnSettings, setColumnSettings] = useState(() => {
    ...
    statusColumns.forEach(col => {
      ...
      initialSettings[col.columnId] = {
        selected: !!watched, // Set true jika ada, false jika tidak
        weight: watched?.weight || 0
      }
    })
  ...
  // 3. PERBAIKI: useEffect juga harus menginisialisasi SEMUA kolom status
  useEffect(() => {
    if (column) {
      const newSettings = {}

      // Gunakan statusColumns yang sudah di-memoize
      statusColumns.forEach(col => {
        const watched = findWatched(col.columnId)

        newSettings[col.columnId] = {
          selected: !!watched, // Set true jika ada, false jika tidak
          weight: watched?.weight || 0
        }
      })
  ...
  }, [column, statusColumns]) // Dependensi pada column dan statusColumns

  // 4. PERBAIKI: handleToggle dibuat immutable
  const handleToggle = columnId => {
    setColumnSettings(prev => {
      const newSettings = { ...prev }

      // newSettings[columnId] sekarang dijamin ada
      if (newSettings[columnId]) {
        // --- INI PERBAIKAN IMUTABILITAS ---
        // Buat salinan baru dari objek di dalamnya
        newSettings[columnId] = {
          ...newSettings[columnId], // Salin properti lama (seperti weight)
          selected: !newSettings[columnId].selected // Ubah nilai selected
        }
      }
  ...
  // 5. PERBAIKI: handleWeightChange dibuat immutable
  const handleWeightChange = (columnId, weight) => {
    setColumnSettings(prev => {
      ...
      if (newSettings[columnId]) {
        // --- INI PERBAIKAN IMUTABILITAS ---
        newSettings[columnId] = {
          ...newSettings[columnId], // Salin properti lama (seperti selected)
          weight: Math.max(0, Math.min(100, parseInt(weight) || 0))
        }
      }
  ...
  const handleDistributeEqually = () => {
    ...
    setColumnSettings(prev => {
      ...
      Object.keys(newSettings).forEach(colId => {
        // Cek apakah objeknya ada dan terpilih
        if (newSettings[colId] && newSettings[colId].selected) {
          // Buat salinan baru saat update
          newSettings[colId] = {
            ...newSettings[colId],
            weight: equalWeight + (first ? remainder : 0)
          }
          first = false
        }
      })
...
export default function TableView({ board }) {
  ...
  // HANDLER BARU: Toggle hide/show
  const handleToggleColumnVisibility = columnId => {
    setHiddenColumnIds(prev => {
      if (prev.includes(columnId)) {
        return prev.filter(id => id !== columnId) // Show
      } else {
        return [...prev, columnId] // Hide
      }
    })
  }

  // HANDLER BARU: Hide kolom dari menu kebab
  ...
  const handleSaveProgressSettings = async statusColumnsData => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Jika API berhasil, revalidate untuk mendapatkan data terbaru dari server
        mutate(boardApiUrl)
      }
    } catch (error) {
      // Jika fetch gagal, kembalikan UI ke data asli
