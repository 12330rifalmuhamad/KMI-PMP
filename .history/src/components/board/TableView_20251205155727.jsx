...
// =================================================================
// REUSABLE BOARD CELL (UNTUK MAIN ITEM & SUBITEM)
// =================================================================
const BoardCell = ({ item, column, board, onUpdateValue, onClick }) => {
  ...
  // Wrapper agar onClick bisa diteruskan (untuk trigger popover edit di parent)
  const handleClick = e => {
...
const SubItemsView = ({ parentItem, board, columns, onUpdateValue, onCreateSubitem, onCellClick }) => {
  ...
  return (
    <Box sx={{ width: '100%', pl: 5, pr: 0, py: 0.5 }} className='bg-gray-50/50 dark:bg-slate-900/50'>
      <div className='flex relative'>
        ...
        <div className='w-full bg-white dark:bg-gray-800 border-l border-divider overflow-hidden shadow-sm z-10'>
          <table className='min-w-full border-collapse'>
            {/* Header Subitem (Mengikuti Kolom Utama) */}
            <thead>
              <tr className='bg-gray-100 dark:bg-gray-700 border-b border-divider'>
                {columns.map(col => (
                  <th
                    key={col.columnId}
                    className='px-2 py-1 text-[10px] font-bold text-gray-500 uppercase border-r border-divider text-left'
                  >
                    {col.columnName}
                  </th>
                ))}
            ...
            <tbody>
              {subItems.map(sub => (
                <tr
                  key={sub.taskId}
                  className='group hover:bg-gray-50 dark:hover:bg-gray-700/50 border-b border-divider last:border-0'
                >
                  {columns.map(col => (
                    <td
                      key={`${sub.taskId}-${col.columnId}`}
                      className='p-0 border-r border-divider h-8 relative cursor-pointer'
                    >
                      <BoardCell
                        item={sub}
                        column={col}
                        ...
                        onClick={e => onCellClick(e, sub, col)} // Trigger popover edit
                      />
                    </td>
                  ))}
              ...
              {/* Input New Subitem */}
              <tr>
                <td className='border-r border-divider p-0 h-8' colSpan={1}>
                  <input
                    type='text'
                    className='w-full h-full px-3 text-sm bg-transparent outline-none placeholder:text-gray-400 focus:bg-white dark:focus:bg-gray-700 transition-colors'
...
// ... (Kode Editor Popover sama seperti sebelumnya, tidak diubah) ...
                        ...
                        <tr className='bg-backgroundPaper'>
                          ...
                          <td colSpan={visibleColumns.length} className='p-0 border-r border-divider relative'>
                            <SubItemsView
                              parentItem={item}
                              board={board}
                              columns={visibleColumns} // Pass kolom board utama ke subitem
