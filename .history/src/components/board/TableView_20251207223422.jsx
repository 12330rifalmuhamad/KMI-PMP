...
// =================================================================
// DYNAMIC SUBITEMS VIEW (THEME ADAPTIVE)
// =================================================================
  ...
  return (
    <Box sx={{ width: '100%', pl: 5, pr: 0, py: 1, backgroundColor: 'background.default' }}>
      <div className='flex relative'>
        {/* Garis Konektor L-Shape (Adaptive Border Color) */}
        <Box
          sx={{
            position: 'absolute',
            left: -12,
        ...
        >
          <table className='min-w-full border-collapse'>
            {/* Header Subitem (Adaptive Background) */}
            <Box
              component='thead'
              sx={{
                backgroundColor: 'action.hover', // Uses Vuexy's hover color for subtle differentiation
                borderBottom: 1,
                borderColor: 'divider'
              }}
            >
              ...
              {subItems.map(sub => (
                <Box
                  ...
                  sx={{
                    ...
                    '&:hover': { backgroundColor: 'action.hover' }, // Adaptive hover
              ...
              <tr>
                ...
                >
                  <input
                    ...
                    style={{ color: 'var(--mui-palette-text-primary)' }} // Ensure text color adapts
                  />
                </Box>
        ...
        return (
          <div className='p-2'>
            <TextField
              ...
              onKeyDown={e => {
                // Modified: Only save on Enter if NOT TEXT (allows multiline for text)
                if (e.key === 'Enter' && column.columnType !== 'TEXT') {
                  onValueSelect(e.currentTarget.value)
                  onClose()
                } else if (e.key === 'Enter' && e.shiftKey) {
                  // Optional: Shift+Enter to save for TEXT
                  e.preventDefault()
                  onValueSelect(e.currentTarget.value)
                  onClose()
                }
...
export default function TableView({ board }) {
  ...
  const handleSaveTextValue = () => {
    ...
    // Clear editing state is handled by parent or by re-render, but usually we just leave it or set null
                          ...
                          switch (column.columnType) {
                            ...
                            case 'TEXT':
                              if (column.columnName.toLowerCase() === 'item') {
                                ...
                                // Tampilkan value text dengan white-space pre-wrap agar enter terbaca
                              ...
                              >
                                {isEditingItemTitle ? (
                                  ...
                                  <TextField
                                    autoFocus
                                    fullWidth
                                    multiline={column.columnType === 'TEXT'} // Allow multiline for TEXT type
                                    minRows={1}
                                    maxRows={4}
                                    variant='standard'
                                    type={column.columnType === 'NUMBER' ? 'number' : 'text'}
                                    ...
                                    onKeyDown={e => {
                                      ...
                                      if (column.columnType === 'TEXT') {
                                        // Allow normal Enter for new line, prevent default submission
                                        if (e.key === 'Enter' && !e.shiftKey) {
                                          e.stopPropagation()
                                        }

                                        // Optional: Shift+Enter to save
                                        if (e.key === 'Enter' && e.shiftKey) {
                                          e.preventDefault()
                                          handleSaveTextValue()
                                        }
                                      } else {
                                        // For non-text inputs (like number), Enter saves
