...
const ValueEditorPopover = ({ anchorEl, onClose, column, board, onValueSelect }) => {
  ...
  // =================================================================
  // PERBAIKAN: Fungsi handleSaveLabels
  // =================================================================
  const handleSaveLabels = async () => {
    setIsLoading(true)
    const boardApiUrl = `/api/boards/${board.boardId}`
    const originalBoardData = structuredClone(board) // Simpan data asli untuk rollback
    ...
    // 3. Mutate UI secara instan (Optimistic Update)
    mutate(boardApiUrl, newBoardData, { revalidate: false })
    setIsEditingLabels(false)
    // --- Selesai Optimistic UI ---

    try {
      // 4. Kirim request ke backend di latar belakang
      ...
      if (!response.ok) {
        // 5. Jika API gagal, kembalikan UI ke data asli (Rollback)
      ...
      // 6. Jika BERHASIL, panggil mutate() TANPA data untuk re-sync
      //    Ini akan mengambil ID baru dari DB jika diperlukan.
      else {
        mutate(boardApiUrl)
      }
    } catch (error) {
      ...
      alert('Gagal menyimpan label.')
      // 7. Jika fetch gagal, kembalikan UI ke data asli (Rollback)
      mutate(boardApiUrl, originalBoardData, { revalidate: false })
    } finally {
      setIsLoading(false)
    }
...
export default function TableView({ board }) {
  ...
  // =================================================================
  const handleSaveProgressSettings = async watchedColumnIds => {
    ...
    columnToUpdate.watchedStatusColumns = watchedColumnIds.map(id => {
      const statusCol = board.columns.find(c => c.columnId === id)
      return {
    ...
    try {
      const response = await fetch(`/api/columns/${columnId}/progress-settings`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ statusColumnIds: watchedColumnIds })
      })
                    ...
                    if (column.columnName.toLowerCase() === 'item') {
                      ...
                      if (watchedColumnIds.length > 0) {
                        ...
                        group.items.forEach(item => {
                          ...
                          totalPercent += (doneCount / watchedColumnIds.length) * 100
                        })
