...
const getColumnIcon = type => {
  switch (type) {
    ...
    default:
      return 'tabler-layout-columns'
  }
}

// =================================================================
// CELL COMPONENTS (Sama seperti sebelumnya)
// =================================================================
...
// =================================================================
// NEW COMPONENT: SORT POPOVER
// =================================================================
const SortPopover = ({ anchorEl, onClose, columns, sorts, setSorts }) => {
  const open = Boolean(anchorEl)

  const handleAddSort = () => {
    // Default to 'item_title' if no sorts exist, or first available column
    setSorts([...sorts, { id: Date.now(), columnId: 'item_title', direction: 'asc' }])
  }
  ...
  const handleSortChange = (id, field, value) => {
  ...
  return (
    <Popover
      ...
      anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }}
      transformOrigin={{ vertical: 'top', horizontal: 'left' }}
      PaperProps={{ sx: { width: 500, p: 0, borderRadius: 2 } }}
    >
      <Box
        sx={{
          p: 2,
      ...
      >
        <Box className='flex items-center gap-2'>
          <i className='tabler-arrows-sort text-textSecondary' />
          <Typography variant='subtitle1' fontWeight='bold'>
            Sort by
          </Typography>
        </Box>
      ...
      <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
        {sorts.length === 0 ? (
          <Box className='flex flex-col items-center justify-center py-4 text-textSecondary gap-2'>
            <Typography variant='body2'>No sorts applied.</Typography>
            <Button startIcon={<i className='tabler-plus' />} onClick={handleAddSort} variant='outlined' size='small'>
              Add sort
            </Button>
          </Box>
        ) : (
          sorts.map((sort, index) => (
            <Box key={sort.id} sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
              <Typography variant='body2' sx={{ width: 80, color: 'text.secondary', fontWeight: 'medium' }}>
              ...
              <FormControl size='small' sx={{ flex: 1 }}>
                <Select
                  value={sort.columnId}
                  onChange={e => handleSortChange(sort.id, 'columnId', e.target.value)}
                  displayEmpty
                  renderValue={selected => {
                    if (selected === 'item_title')
                      return (
                        <Box className='flex items-center gap-2'>
                          <i className='tabler-t text-textSecondary' /> Name
                        </Box>
                      )
                    const col = columns.find(c => c.columnId === selected)

                    return col ? (
                      <Box className='flex items-center gap-2'>
                        <i className={`${getColumnIcon(col.columnType)} text-textSecondary`} /> {col.columnName}
                      </Box>
                    ) : (
                      'Unknown'
                    )
                ...
                >
                  <MenuItem value='item_title'>
                    <ListItemIcon>
                      <i className='tabler-t' />
                    </ListItemIcon>{' '}
                    Name
                  </MenuItem>
                  {columns.map(col => (
                    <MenuItem key={col.columnId} value={col.columnId}>
                      <ListItemIcon>
                        <i className={getColumnIcon(col.columnType)} />
                      </ListItemIcon>
                      {col.columnName}
                    </MenuItem>
                  ))}
              ...
              <FormControl size='small' sx={{ width: 140 }}>
                <Select
                  ...
                  renderValue={selected => (
                    <Box className='flex items-center gap-2'>
                      <i className={selected === 'asc' ? 'tabler-sort-ascending' : 'tabler-sort-descending'} />{' '}
              ...
              <IconButton size='small' onClick={() => handleRemoveSort(sort.id)}>
                <i className='tabler-x' />
              </IconButton>
            </Box>
          ))
...
export default function TableView({ board }) {
  ...
  const processedGroups = useMemo(() => {
      ...
      resultGroups = resultGroups.map(group => {
        const sortedItems = [...group.items].sort((a, b) => {
          for (const sort of sorts) {
            ...
            // Get Value A
            if (sort.columnId === 'item_title') valA = a.taskTitle || ''
            else {
              const v = a.values.find(val => normalizeId(val.columnId) === normalizeId(sort.columnId))

              valA = v ? v.value : ''
            }

            // Get Value B
            if (sort.columnId === 'item_title') valB = b.taskTitle || ''
            else {
              const v = b.values.find(val => normalizeId(val.columnId) === normalizeId(sort.columnId))

              valB = v ? v.value : ''
            }

            // Check if column is number
            const colDef = board.columns.find(c => c.columnId === sort.columnId)
            const isNum = colDef?.columnType === 'NUMBER'
    ...
    return resultGroups
  }, [groups, filters, sorts, board.columns])
    ...
    <div className='relative'>
      ...
      <div className='flex justify-between items-center mb-2'>
        <div className='flex gap-2'>
          ...
          {/* SORT BUTTON */}
          <Button
            variant={sorts.length > 0 ? 'contained' : 'outlined'}
            size='small'
            startIcon={<i className='tabler-arrows-sort' />}
            onClick={e => setSortMenuAnchor(e.currentTarget)}
            className={`!border-divider ${sorts.length === 0 ? '!text-textSecondary' : ''}`}
          >
