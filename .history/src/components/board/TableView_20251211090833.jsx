  ...
  return (
    <div className='flex flex-wrap gap-1 px-2 items-center h-full overflow-hidden'>
      {tags.map((tagLabel, index) => {
        const option = column.options?.find(opt => opt.label === tagLabel.trim()) // Added trim
...
const TagsPopover = ({ anchorEl, onClose, item, column, onSave, onAddOption, onManage }) => {
  ...
  // 1. Initialize State from Props (Lazy init with TRIM and FILTER)
  const [selectedTags, setSelectedTags] = useState(() => {
    const val = (item?.values || []).find(v => v.columnId === column?.columnId)?.value

    // FIX: Trim whitespace and filter out empty strings
    return val
      ? val
          .split(',')
          .map(t => t.trim())
  ...
  const handleToggleTag = label => {
    const trimmedLabel = label.trim() // Normalize label
    let newTags
    ...
    >
      ...
      <List dense sx={{ maxHeight: 200, overflowY: 'auto', py: 0 }}>
        {filteredTags.map(tag => {
          // KEY FIX: Check against trimmed labels
          ...
          return (
            <ListItemButton key={tag.label} onClick={() => handleToggleTag(tag.label)} dense>
              {/* CHECKBOX controlled by local state */}
...
const ManageTagsDialog = ({ open, onClose, column, onUpdateOption, onDeleteOption }) => {
  ...
  const handleStartEdit = tag => {
    setEditingId(tag.id || tag.label) // Fallback to label if ID missing
    setTempName(tag.label)
  }

  const handleSaveEdit = tag => {
    if (tempName && tempName !== tag.label) {
      onUpdateOption({ ...tag, label: tempName }, tag) // Pass old object as 2nd arg
    }
  ...
  const handleColorChange = color => {
    ...
    if (tag) {
      onUpdateOption({ ...tag, color }, tag) // Pass old object as 2nd arg
    }
...
export default function TableView({ board }) {
  ...
  const handleModifyColumnOptions = async (columnId, action, payload, originalPayload = null) => {
    ...
    setHeaderColumns(prev =>
      prev.map(col => {
        if (col.columnId === columnId) {
          ...
          if (action === 'UPDATE') {
            // Fix for rename: Use ID or fallback to original label to find the correct item to update
    ...
    let newOptions = updatedColumn?.options || [] // Fallback to current options if not found

    // Re-calculate new options array based on action (same logic as above but for API payload)
  ...
  // =================================================================
  // Helper to ensure Popover always gets fresh item data even if state is stale
