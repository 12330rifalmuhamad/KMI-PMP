...
// SUBITEM COMPONENT
// =================================================================
const SubItemsView = ({ parentItem, board, onUpdateSubitem, onCreateSubitem }) => {
  const [newSubTitle, setNewSubTitle] = useState('')

  // Mapping subitems dari parentItem (Asumsi struktur data: item.subItems)
  const subItems = parentItem.subItems || []

  // Definisi Kolom Subitem (Hardcoded sesuai referensi gambar untuk MVP)
  // Anda bisa membuatnya dinamis jika backend mendukung "Subitem Columns"
  const subColumns = [
    { label: 'Subitem', width: '40%' },
  ...
  return (
    <Box sx={{ width: '100%', pl: 6, pr: 2, py: 1, bgcolor: '#f5f7fa' }} className='dark:bg-slate-900'>
      <div className='flex relative'>
        {/* Garis Konektor L-Shape */}
        <div className='absolute -left-4 top-[-20px] bottom-4 w-4 border-l-2 border-b-2 border-gray-300 rounded-bl-xl'></div>

        <div className='w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md overflow-hidden shadow-sm'>
          {/* Subitem Header */}
          <div className='flex border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700'>
            {subColumns.map((col, idx) => (
              <div
                key={idx}
                className='px-3 py-1 text-xs font-semibold text-gray-500 uppercase border-r border-gray-200 dark:border-gray-600 last:border-0'
                style={{ width: col.width }}
              >
                {col.label}
              </div>
            ))}
            <div className='w-8'></div> {/* Spacer untuk tombol add/delete */}
          </div>

          {/* Subitem List */}
          {subItems.map(sub => (
            <div
              key={sub.subTaskId || sub.id}
              className='flex border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 group'
            >
              {/* Kolom Nama Subitem */}
              <div
                className='px-3 py-1.5 flex items-center border-r border-gray-100 dark:border-gray-700'
                style={{ width: '40%' }}
              >
              ...
              {/* Kolom Owner (Person) */}
              <div
                className='px-3 py-1.5 flex justify-center items-center border-r border-gray-100 dark:border-gray-700'
                style={{ width: '15%' }}
              >
                <div className='w-6 h-6 rounded-full bg-gray-300 flex items-center justify-center text-xs'>
                  <i className='tabler-user text-white'></i>
                </div>
              </div>

              {/* Kolom Status (Dummy UI) */}
              <div
                className='px-1 py-1 flex items-center justify-center border-r border-gray-100 dark:border-gray-700'
                style={{ width: '20%' }}
              >
                <div className='w-full h-5 bg-gray-200 dark:bg-gray-600 rounded text-[10px] flex items-center justify-center text-gray-500'>
                  -
                </div>
              </div>
              ...
              {/* Kolom Date */}
              <div
                className='px-3 py-1.5 flex justify-center items-center text-xs text-gray-500 border-r border-gray-100 dark:border-gray-700'
                style={{ width: '20%' }}
              >
                -
              </div>
              ...
              <div className='w-8 flex items-center justify-center opacity-0 group-hover:opacity-100'>
                <i className='tabler-trash text-red-400 cursor-pointer text-xs'></i>
              </div>
            </div>
          ))}
    ...
    return filteredData.map(group => {
      const sortedItems = [...group.items].sort((a, b) => {
        ...
        if (sortConfig.columnId === 'item_title') {
          ...
          const v = a.values.find(val => normalizeId(val.columnId) === normalizeId(sortConfig.columnId))
        ...
        if (sortConfig.columnId === 'item_title') {
          ...
          const v = b.values.find(val => normalizeId(val.columnId) === normalizeId(sortConfig.columnId))

          valB = v ? v.value : ''
        }

        // Cek Type Column untuk sorting yang tepat
                          ...
                          switch (column.columnType) {
                            ...
                            case 'TEXT':
                              if (column.columnName.toLowerCase() === 'item') {
                                // CELL CONTENT KHUSUS UNTUK ITEM NAME (DENGAN CHEVRON)
                                cellContent = (
