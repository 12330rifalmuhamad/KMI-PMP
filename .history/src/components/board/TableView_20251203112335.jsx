...
export default function TableView({ board }) {
  ...
  // Placeholder: Fungsi untuk membuat Sub Item (perlu disesuaikan dengan API Anda)
  const handleCreateSubTask = parentTaskId => {
    alert(`Create subtask for parent: ${parentTaskId}. (API Endpoint needed)`)

    // Contoh implementasi logika:
    // await fetch(`/api/tasks/${parentTaskId}/subtasks`, ...)
  }
  ...
  const handleUpdateValue = async (itemToUpdate, columnToUpdate, newValue) => {
    ...
    // 1. UPDATE SWR Cache (Global) - Logic need to be recursive for subitems ideally, simplified here
    const group = newBoardData.groups.find(g => g.groupId === itemToUpdate.groupId)

    if (group) {
      // Find in items OR subItems
      let item = group.items.find(i => i.taskId === itemToUpdate.taskId)

      // If not found in main items, try searching subitems (Logic added)
      if (!item) {
        for (const pItem of group.items) {
          if (pItem.subItems) {
            const sub = pItem.subItems.find(s => s.taskId === itemToUpdate.taskId)
    ...
    // (Simplified for brevity, assuming SWR mutation handles UI responsiveness mostly)
    setEditingCell(null)
      ...
      <React.Fragment key={item.taskId}>
            ...
            <div className='flex justify-center items-center h-full relative'>
              ...
              {!isSubItem && (
                <div className='absolute left-0 h-full flex items-center justify-center w-6'>
                  {/* Jika ada sub item atau kita ingin membolehkan add subitem, tampilkan chevron */}
                  <IconButton
                    size='small'
        ...
        {isExpanded &&
          ...
          item.subItems.map(subItem =>
            renderRow(subItem, group, true) // Recursive call with isSubItem = true
          )}
