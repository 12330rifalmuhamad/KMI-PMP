...
const SubItemsView = ({ parentItem, board, columns, onUpdateValue, onCreateSubitem, onCellClick }) => {
  ...
  // Styles angepasst für Vuexy Dark/Light
  return (
    <Box sx={{ width: '100%', pl: 5, pr: 0, py: 1 }} className='bg-[#F8F7FA] dark:bg-[#25293C]'>
...
const ProgressColumnSettingsModal = ({ open, onClose, board, column, onSave }) => {
  ...
  const [columnSettings, setColumnSettings] = useState(() => {
    ...
    statusColumns.forEach(c => {
      const w = findWatched(c.columnId)

      i[c.columnId] = { selected: !!w, weight: w?.weight || 0 }
    })
  ...
  useEffect(() => {
    if (column) {
      ...
      statusColumns.forEach(c => {
        ...
        n[c.columnId] = { selected: !!w, weight: w?.weight || 0 }
      })
      setColumnSettings(n)
    }
  }, [column, statusColumns])
  const handleToggle = id => setColumnSettings(p => ({ ...p, [id]: { ...p[id], selected: !p[id].selected } }))

  const handleWeightChange = (id, v) =>
    setColumnSettings(p => ({ ...p, [id]: { ...p[id], weight: Math.max(0, Math.min(100, parseInt(v) || 0)) } }))

  const handleSave = async () => {
    setIsLoading(true)

    try {
      const d = Object.entries(columnSettings)
        .filter(([_, v]) => v.selected)
        .map(([id, v]) => ({ statusColumnId: parseInt(id), weight: v.weight }))
    ...
    <Modal open={open} onClose={onClose}>
      <Card className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-backgroundPaper p-4'>
        <CardContent className='flex flex-col gap-4'>
          ...
          <FormGroup>
            {statusColumns.map(c => (
              <Box key={c.columnId} className='flex gap-4 items-center'>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={columnSettings[c.columnId]?.selected}
                      onChange={() => handleToggle(c.columnId)}
                    />
                  }
                  label={c.columnName}
                  className='flex-1'
                />
                {columnSettings[c.columnId]?.selected && (
                  <TextField
                    type='number'
                    size='small'
                    value={columnSettings[c.columnId]?.weight}
                    onChange={e => handleWeightChange(c.columnId, e.target.value)}
                    sx={{ width: 80 }}
                  />
                )}
              </Box>
                              ...
                              <div className='flex items-center gap-2 w-full'>
                                ...
                                <div className='flex-1 min-w-0'>
                                  {editingTaskName?.taskId === item.taskId ? (
                                    ...
                                    <div
                                      className='truncate font-medium cursor-pointer text-gray-700 dark:text-[#E1DEF5]'
                                      onClick={e => {
                                        e.stopPropagation()
