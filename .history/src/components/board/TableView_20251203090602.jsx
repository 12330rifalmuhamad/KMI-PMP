...
// Pastikan Anda sudah membuat file-file ini di folder yang sama
...
const LinkCell = ({ value }) => {
  ...
  return (
    <a
      ...
      className='text-blue-400 hover:text-blue-300 underline truncate block px-2'
...
const ProgressCell = ({ item, column }) => {
  const progress = useMemo(() => {
    ...
    watchedColumnIds.forEach(colId => {
      ...
      if (taskValue && DONE_LABELS.includes(taskValue.value)) doneCount++
    })
  ...
  const handleSaveLabels = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Jangan revalidate untuk menghindari flicker
      }
    } catch (error) {
                    ...
                    {headerColumns.map(column => {
                      ...
                      switch (column.columnType) {
                        ...
                        case 'NUMBER':
                          ...
                          break
                        case 'PROGRESS':
                          cellContent = <ProgressCell item={item} column={column} allColumns={board.columns} />
                          break
                        case 'TAGS':
