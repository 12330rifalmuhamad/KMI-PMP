...
export default function TableView({ board }) {
  ...
  const handleToggleSelectAll = () => {
    if (selectedTaskIds.length === allTaskIds.length) {
      setSelectedTaskIds([]) // Unselect all
    } else {
      setSelectedTaskIds(allTaskIds) // Select all
    }
  }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      // Loop delete request (karena API bulk delete belum tentu ada, kita loop fetch)
      await Promise.all(selectedTaskIds.map(id => fetch(`/api/tasks/${id}`, { method: 'DELETE' })))
      mutate(`/api/boards/${board.boardId}`)
    } catch (error) {
      console.error('Bulk delete failed', error)
      setGroups(originalGroups) // Revert jika gagal
  ...
  const renderHeaderRow = (isMainHeader = false) => {
    // --- MODIFIED: HEADER CHECKBOX LOGIC ---
  ...
  return (
    <div className='relative'>
                    ...
                    >
                      <td className='sticky left-0 z-10 bg-backgroundPaper p-0 w-12 border-r border-divider'>
                        <div className='flex justify-center items-center h-full relative'>
                          {/* LOGIKA CHECKBOX & KEBAB:
                            1. Checkbox: Muncul jika selected ATAU saat hover.
                            2. Kebab: Muncul saat hover JIKA item tidak selected.
                          */}
                          <div
      ...
      {/* --- MODIFIED: FLOATING ACTION BAR FOR BULK ACTIONS --- */}
