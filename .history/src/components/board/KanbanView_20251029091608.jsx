...
const KanbanView = ({ board }) => {
  ...
  // Use memoized columns as base, with local state for drag-and-drop
  const columns = localColumns.length > 0 ? localColumns : kanbanColumns

  // Update local columns when kanbanColumns changes
  useEffect(() => {
  ...
  const handleTaskMove = (movedTaskId, fromColumnId, toColumnId) => {
    ...
    console.log('🎯 handleTaskMove called:', movedTaskId, fromColumnId, toColumnId)
    ...
    console.log('🎯 Updating status to:', newStatus, 'for columnId:', statusColumnId)
