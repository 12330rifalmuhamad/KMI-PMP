...
const KanbanView = ({ board }) => {
  ...
  const { kanbanColumns, tasksMap } = useMemo(() => {
    ...
    console.log('Person column:', personColumn)
    console.log('Date column:', dateColumn)

    if (!statusColumn) {
      console.log('No Status column found, returning empty kanban')
    ...
    board.groups.forEach(group => {
      group.items.forEach(item => {
        ...
        console.log(`Task ${item.taskId}: rawStatus = ${rawStatus}`)
        ...
        console.log(`Task ${item.taskId}: final status = ${statusValue}`)
    ...
    console.log('Final kanban columns:', columns)
    console.log('Tasks map:', allTasks)
  ...
  const handleTaskMove = (movedTaskId, fromColumnId, toColumnId) => {
    ...
    console.log(`Moving task ${movedTaskId} from ${fromColumnId} to ${toColumnId}`)
  ...
  return (
    <div className='flex flex-col h-full'>
      <div className='flex-1 overflow-x-auto overflow-y-hidden'>
        <div ref={boardRef} className='flex gap-6 min-w-max h-full pb-4'>
          {columns.map(column => (
            <KanbanList
              key={column.id}
              column={column}
