...
const TimelineCell = ({ value, column }) => {
  ...
  if (!start || !end) return <span className='text-[10px] text-red-400'>Set Dates</span>
  ...
  // Timeline Green Color (Monday.com style)
  const bgColor = 'bg-[#00c875]'
  const hoverColor = 'hover:bg-[#00b569]'

  return (
    <div className='w-full px-2 py-1 h-full flex items-center'>
      <div
        className={`flex items-center justify-center text-[11px] text-white font-medium rounded-full ${bgColor} ${hoverColor} h-6 w-full truncate px-2 cursor-pointer transition-colors relative`}
        title={displayRange}
      >
    ...
    >
      <Box sx={{ p: 2 }}>
        <div className='flex justify-between items-center mb-4'>
          ...
          <Typography variant='caption' color='textSecondary' className='bg-gray-100 px-2 py-0.5 rounded'>
        ...
        <div className='flex items-center justify-between'>
          <FormControlLabel
            ...
            label={<Typography variant='body2'>Set as milestone</Typography>}
          />
...
export default function TableView({ board }) {
  ...
  // Sync prop board ke local state groups jika board berubah (dari SWR)
  useEffect(() => {
    if (board?.groups) {
  ...
  // 1. General Update (Cell Value & Title)
  const handleGeneralUpdate = async (itemToUpdate, columnOrType, newValue) => {
    const columnIdNumber = Number(columnOrType.columnId)
    const sanitizedValue = String(newValue ?? '')
    const isTitleUpdate = columnOrType.columnType === 'TITLE'
    ...
    const newGroups = groups.map(g => ({
      ...
      items: g.items.map(i => {
        // Logic Update Title
        if (isTitleUpdate) {
          if (i.taskId === itemToUpdate.taskId) return { ...i, taskTitle: sanitizedValue }
            ...
            if (subFound) {
              return {
                ...
                subItems: i.subItems.map(s =>
                  s.taskId === itemToUpdate.taskId ? { ...s, taskTitle: sanitizedValue } : s
                )
              }
            }
          }
        ...
        // Logic Update Values (Cells)
        const updateValues = values => {
          const existing = values.find(v => normalizeId(v.columnId) === normalizeId(columnOrType.columnId))

          if (existing)
            return values.map(v =>
              normalizeId(v.columnId) === normalizeId(columnOrType.columnId) ? { ...v, value: sanitizedValue } : v
            )

          return [...values, { columnId: columnIdNumber, value: sanitizedValue }]
        }

        if (i.taskId === itemToUpdate.taskId) return { ...i, values: updateValues(i.values || []) }

        // Handle Subitems Values
        if (i.subItems) {
          const subFound = i.subItems.find(s => s.taskId === itemToUpdate.taskId)
    ...
    setGroups(newGroups) // Render UI update instantly!
    ...
    try {
      if (isTitleUpdate) {
        await fetch(`/api/tasks/${itemToUpdate.taskId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ taskTitle: sanitizedValue })
        })
      } else {
      ...
      // STEP 3: Silent Revalidate (Keep UI stable)
      mutate(`/api/boards/${board.boardId}`, undefined, { revalidate: false })
    } catch (e) {
      console.error('Update failed', e)
      setGroups(board?.groups || []) // Rollback
    }
  }

  // 2. Optimistic Create Task
  const handleCreateTask = async groupId => {
    const tempId = `temp-${Date.now()}`
    const title = newItemTitle || 'New Item'

    const newGroups = groups.map(g => {
      if (g.groupId === groupId) {
        return {
          ...g,
          items: [...g.items, { taskId: tempId, taskTitle: title, values: [], subItems: [] }]
        }
      }
    ...
    try {
      await fetch(`/api/groups/${groupId}/tasks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ txtTaskTitle: title })
      })
      mutate(`/api/boards/${board.boardId}`)
    } catch (e) {
      console.error(e)
  ...
  const handleDeleteTask = async taskId => {
    ...
    const newGroups = groups.map(g => ({
      ...g,
      items: g.items.filter(i => i.taskId !== taskId)
    }))
  ...
  // Wrapper handlers
  const handleUpdateValue = (item, column, value) => handleGeneralUpdate(item, column, value)
  ...
  const handleCreateSubitem = async (parentTaskId, title) => {
    try {
      ...
      console.error('Failed create subitem', error)
  ...
  const handleChangeColumnType = async () => {
    if (!activeColumn) return
    const allowed = ['TEXT', 'STATUS', 'PERSON', 'DATE', 'NUMBER', 'CHECKBOX', 'FILES', 'LINK', 'PROGRESS', 'TIMELINE']
                        ...
                        {visibleColumns.map(column => {
                          ...
                          switch (column.columnType) {
                            ...
                            case 'PROGRESS':
                              ...
                              break
                            case 'TIMELINE':
                              cellContent = <TimelineCell value={cellValue?.value} column={column} />
                              break
                            case 'TEXT':
