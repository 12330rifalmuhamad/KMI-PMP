...
const ProgressColumnSettingsModal = ({ open, onClose, board, column, onSave }) => {
  ...
  // State untuk menyimpan selected columns dengan weight
  const [columnSettings, setColumnSettings] = useState(() => {
    const initialSettings = {}

    if (column?.watchedStatusColumns) {
  ...
  // Update columnSettings ketika column berubah
  useEffect(() => {
    if (column) {
      const newSettings = {}
      ...
      if (column.watchedStatusColumns) {
        column.watchedStatusColumns.forEach(link => {
          newSettings[link.statusColumnId] = {
            selected: true,
            weight: link.weight || 0
          }
        })
  ...
  const handleToggle = columnId => {
    setColumnSettings(prev => {
      ...
      if (newSettings[columnId]) {
        ...
        // Kolom baru dipilih, set default weight 0
    ...
    setColumnSettings(prev => {
      ...
      if (newSettings[columnId]) {
        newSettings[columnId] = {
          ...
          weight: Math.max(0, Math.min(100, parseInt(weight) || 0)) // Ubah nilai weight
        }
      }
