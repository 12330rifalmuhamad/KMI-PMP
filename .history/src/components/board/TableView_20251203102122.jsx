...
export default function TableView({ board }) {
  ...
  const handleUpdateValue = async (itemToUpdate, columnToUpdate, newValue, isSubitem = false) => {
    ...
    // NOTE: This logic needs to traverse groups -> items -> subitems if isSubitem is true.
    // For simplicity, we trigger SWR revalidation after API call for subitems, or improve deep clone logic.

    if (!isSubitem) {
      // ... (Existing Optimistic Logic for Main Items) ...
      ...
      setGroups(newBoardData.groups) // Update local state immediately
    } else {
      // Optimistic update for subitems (Simplified: just refetch)
      // Ideally: Find parent item, find subitem, update value.
    }
    ...
    const endpoint = isSubitem
      ? `/api/subitems/${itemToUpdate.subitemId}/values` // Assumed endpoint
  ...
  const handleCreateSubitem = async parentTaskId => {
    ...
    // POST /api/tasks/${parentTaskId}/subitems
    await handleApiCall(
  ...
  // Updated save handler to support subitems
  ...
  // =================================================================
  // RENDER ROW COMPONENT (Untuk Item & Subitem agar DRY)
  // =================================================================
  const RenderRow = ({ item, isSubitem = false }) => {
    return (
      <tr
        key={isSubitem ? item.subitemId : item.taskId}
        className={`hover:bg-action-hover group ${isSubitem ? 'bg-gray-50' : ''}`}
      >
        {/* Column 1: Checkbox & Menu */}
        <td
        ...
        {visibleColumns.map((column, colIndex) => {
          ...
          switch (column.columnType) {
            ...
            case 'TEXT':
              if (isItemTitleCol) {
                // Logic judul item + Subitem Toggle
                cellContent = (
                  <div className='flex items-center gap-2'>
                    {/* Indentasi untuk subitem */}
                    {isSubitem && (
                      <div className='w-6 border-l-2 border-b-2 h-4 border-gray-300 ml-2 rounded-bl-md'></div>
                    )}

                    {/* Toggle Button untuk Parent Item */}
                    {!isSubitem && (
                      <IconButton
                        size='small'
                        onClick={e => {
                    ...
                    {/* Text Judul */}
                    <span className={isSubitem ? 'text-gray-600 text-sm' : ''}>
                      {isSubitem ? item.title || item.taskTitle : item.taskTitle}
                    </span>

                    {/* Badge count subitem jika collapsed */}
                    ...
                    {expandedItemIds.includes(item.taskId) && (
                      <>
                        ...
                        <tr className='bg-gray-50'>
                          <td className='sticky left-0 z-10 bg-gray-50 p-0 w-12 border-r border-divider'></td>
                          <td className='p-2 text-textSecondary border-r border-divider pl-10'>
                            {activeSubitemInput === item.taskId ? (
                              ...
                              <button
                                ...
                                className='flex items-center w-full text-left text-sm text-gray-500 hover:text-primary'
                              >
