...
const TagsPopover = ({ anchorEl, onClose, item, column, onSave, onAddOption }) => {
  const open = Boolean(anchorEl)
  ...
  return (
    ...
    >
      ...
      <Box sx={{ p: 1, textAlign: 'center' }}>
        <Button size='small' sx={{ textTransform: 'none', color: 'text.secondary', fontSize: '0.75rem' }}>
          Manage tags
        </Button>
      </Box>
    </Popover>
...
export default function TableView({ board }) {
  ...
  const handleAddTagOption = async (columnId, newOption) => {
    ...
    setHeaderColumns(prev =>
      prev.map(col => {
        if (col.columnId === columnId) {
          ...
          if (currentOptions.some(o => o.label === newOption.label)) return col

          return { ...col, options: [...currentOptions, newOption] }
        }
    ...
    try {
      ...
      console.error('Failed to add tag option', e)
    }
  }
    ...
    <div className='relative'>
      ...
      {tagsPopover.anchorEl && (
        <TagsPopover
          ...
          onAddOption={newOption => handleAddTagOption(tagsPopover.column.columnId, newOption)}
        />
      )}
      <Menu
