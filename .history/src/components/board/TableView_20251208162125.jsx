...
// --- PERBAIKAN: Gunakan Local State di TagsPopover agar Checkbox Instan ---
const TagsPopover = ({ anchorEl, onClose, item, column, onSave, onAddOption, onManage }) => {
  const open = Boolean(anchorEl)
  const [searchTerm, setSearchTerm] = useState('')

  // 1. Initialize State from Props (Lazy init)
  ...
  const handleToggleTag = label => {
    ...
    if (selectedTags.includes(label)) {
      newTags = selectedTags.filter(t => t !== label)
    } else {
  ...
  const handleCreateNewTag = () => {
    ...
    onAddOption({ label: searchTerm, color: newColor })
    handleToggleTag(searchTerm)
    setSearchTerm('')
  }
