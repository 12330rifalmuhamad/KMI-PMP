...
const ProgressColumnSettingsModal = ({ open, onClose, board, column }) => {
  ...
  // Ambil semua kolom 'STATUS' di papan ini, kecuali kolom progress itu sendiri
  const statusColumns = board.columns.filter(c => c.columnType === 'STATUS' && c.columnId !== column.columnId)

  // Siapkan state untuk checkbox, berdasarkan pengaturan yang sudah ada
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      mutate(`/api/boards/${board.boardId}`) // Refresh data papan
      onClose()
    } catch (error) {
      console.error(error)
