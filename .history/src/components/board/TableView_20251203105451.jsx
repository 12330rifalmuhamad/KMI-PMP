...
const DateCell = ({ value }) => {
  ...
  return (
    <span className='text-xs'>{new Date(value).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</span>
  )
}
...
const LinkCell = ({ value }) => {
  ...
  return (
    <a
      ...
      className='text-blue-400 hover:text-blue-300 underline truncate text-xs'
  ...
  const RenderRow = ({ item, isSubitem = false }) => {
    return (
      <tr
        ...
        className={`hover:bg-action-hover group ${isSubitem ? 'bg-white' : 'bg-backgroundPaper'}`}
      >
        <td
          className={`p-0 w-12 border-r border-divider border-b ${isSubitem ? 'bg-white' : 'bg-backgroundPaper sticky left-0 z-10'}`}
        >
        ...
        {visibleColumns.map(column => {
                ...
                cellContent = (
                  <div className='flex items-center gap-2 pl-2'>
                    {!isSubitem && (
                      ...
                      >
                        <i
                          className={`tabler-chevron-right transition-transform text-gray-500 ${expandedItemIds.includes(item.taskId) ? 'rotate-90' : ''}`}
                        />
                      </IconButton>
                    )}
                    <span className={isSubitem ? 'text-gray-700 text-sm' : 'text-textPrimary font-medium'}>
                      {isSubitem ? item.title || item.taskTitle : item.taskTitle}
                    </span>
                    {!isSubitem && !expandedItemIds.includes(item.taskId) && item.subitems?.length > 0 && (
                      <span className='ml-2 text-[10px] bg-gray-200 text-gray-600 px-1.5 py-0.5 rounded-full border border-gray-300'>
          ...
          return (
            <td
              ...
              className={`p-0 h-9 border-r border-b border-divider`}
            >
              <div
                onClick={e => {
        ...
        <td className='border-r border-divider border-b'></td>
      </tr>
    )
  }
              ...
              <React.Fragment key={group.groupId}>
                ...
                {group.items?.map(item => (
                  <React.Fragment key={item.taskId}>
                    ...
                    {/* SUBITEMS CONTAINER ROW (Similar to Monday.com visual style) */}
                    {expandedItemIds.includes(item.taskId) && (
                      <tr className='bg-gray-50/50'>
                        <td colSpan={visibleColumns.length + 2} className='p-0 pl-[50px] py-2 relative'>
                          ...
                          <div className='absolute left-[24px] top-[-10px] bottom-[20px] w-[26px] border-l-2 border-b-2 border-gray-300 rounded-bl-xl pointer-events-none z-0'></div>

                          {/* Subitem Card Container */}
                          <div className='bg-white rounded-md shadow-sm border border-gray-200 mr-4 relative z-10 overflow-hidden'>
                            {/* Mini Header for Subitems */}
                            <div className='border-b border-gray-100 bg-gray-50/50 px-3 py-1.5 flex items-center'>
                              <Typography
                                variant='caption'
                            ...
                            <table className='w-full'>
                              <tbody>
                                ...
                                <tr className='bg-white hover:bg-gray-50'>
                                  <td className='p-0 w-12 border-r border-divider'></td>
                                  <td
                                    className='p-2 text-textSecondary border-r border-divider'
                                  ...
                                  >
                                    {activeSubitemInput === item.taskId ? (
                                      ...
                                      <button
                                        ...
                                        className='flex items-center w-full text-left text-sm text-gray-400 hover:text-primary transition-colors py-1'
                                      >
