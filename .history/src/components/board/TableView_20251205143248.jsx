...
const FilesCell = ({ value }) => {
  ...
  return (
    <div className='flex items-center justify-center gap-1 text-textSecondary'>
            <i className='tabler-paperclip' />      <span>{fileCount > 0 ? fileCount : '-'}</span>   {' '}
    </div>
  )
}
        ...
        <CardContent className='flex flex-col gap-4'>
          ...
          <FormControl fullWidth>
            ...
            <Select value={columnType} label='Column Type' onChange={e => setColumnType(e.target.value)}>
              ...
              <MenuItem value={'STATUS'}>Status</MenuItem>              <MenuItem value={'PERSON'}>People</MenuItem>   
                        <MenuItem value={'DATE'}>Date</MenuItem>             {' '}
              <MenuItem value={'NUMBER'}>Numbers</MenuItem>             {' '}
              <MenuItem value={'CHECKBOX'}>Checkbox</MenuItem>              <MenuItem value={'FILES'}>Files</MenuItem> 
...
export default function TableView({ board }) {
  const { mutate } = useSWRConfig() // --- STATES ---
  ...
  const [progressSettingsModal, setProgressSettingsModal] = useState({ open: false, column: null }) // Visibility States

  const [hiddenColumnIds, setHiddenColumnIds] = useState([])
  const [visMenuAnchor, setVisMenuAnchor] = useState(null) // Selection States

  const [selectedTaskIds, setSelectedTaskIds] = useState([])
  const [isBulkDeleting, setIsBulkDeleting] = useState(false) // --- FILTER STATE ---

  const [filters, setFilters] = useState([])
  const [filterMenuAnchor, setFilterMenuAnchor] = useState(null) // Helper untuk mendapatkan semua Task ID
  ...
  const toClientColumnId = columnId => Number(columnId) // --- FILTERING LOGIC ---
  ...
  }, [groups, filters]) // --- HANDLERS (Standard) ---
