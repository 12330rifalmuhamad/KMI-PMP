...
const PersonAvatar = ({ user }) => {
  if (!user) {
    return (
      <MuiAvatar sx={{ width: 24, height: 24, backgroundColor: '#4A4E69', color: '#C9C8CF' }} title='Unassigned'>
        <i className='tabler-user-off' style={{ fontSize: '0.8rem' }} />
      </MuiAvatar>
    )
  }
...
const StatusCell = ({ value, column }) => {
  ...
  if (column.columnName.toLowerCase() === 'prioritas') {
    const colors = option
      ...
      : 'text-gray-400 border border-gray-600 dark:text-gray-400 dark:border-gray-500'
    const borderClass = option && option.color.includes('/10') ? `border border-${option.color.split('-')[1]}-800` : ''

    return (
      <div
        className={`flex items-center justify-center px-2 py-0.5 rounded-full text-[10px] font-medium ${colors} ${borderClass}`}
      >
        {displayText}
      </div>
    )
  ...
  const colors = option ? `${option.color} ${option.text}` : 'bg-gray-400 dark:bg-gray-600 text-white'

  return (
    <div className={`flex items-center justify-center w-full h-full text-[11px] font-bold ${colors}`}>
      {displayText}
    </div>
  )
}
...
const DateCell = ({ value }) => {
  if (!value) return <span className='text-xs text-gray-500 dark:text-gray-400'>—</span>
...
const LinkCell = ({ value }) => {
  if (!value) return <span className='text-gray-500 dark:text-gray-400 text-xs'>—</span>
  let href = value
  ...
  return (
    <a
      ...
      className='text-blue-500 hover:text-blue-400 dark:text-blue-400 dark:hover:text-blue-300 underline truncate text-xs'
...
const ProgressCell = ({ item, column }) => {
  ...
  return (
    <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', gap: 1 }}>
      <Box sx={{ width: '60%', height: 14, backgroundColor: 'divider', borderRadius: 1, overflow: 'hidden' }}>
        <Box
          sx={{
      ...
      <Typography
        variant='caption'
        sx={{ width: '40%', textAlign: 'right', fontSize: '0.7rem' }}
...
export default function TableView({ board }) {
  ...
  const renderHeaderRow = (isMainHeader = false) => {
    return (
      <tr
        ...
        className={`bg-white dark:bg-[#1e1e1e] border-b border-gray-200 dark:border-gray-700 ${!isMainHeader ? 'bg-opacity-50' : ''}`}
      >
        <th
          className={`sticky left-0 top-0 z-30 bg-white dark:bg-[#1e1e1e] p-0 w-12 border-r border-gray-200 dark:border-gray-700`}
        >
        ...
        {visibleColumns?.map(column => (
          <th
            key={column.columnId}
            className={`p-0 text-left text-sm font-semibold text-gray-900 dark:text-gray-100 whitespace-nowrap border-r border-gray-200 dark:border-gray-700 sticky top-0 z-20 bg-white dark:bg-[#1e1e1e] ${isMainHeader ? 'table-column-draggable' : ''}`}
          >
        ...
        <th className='w-12 border-r border-gray-200 dark:border-gray-700 sticky top-0 z-20 bg-white dark:bg-[#1e1e1e]'>
          {isMainHeader && (
  ...
  const RenderRow = ({ item, isSubitem = false }) => {
    return (
      <tr
        ...
        className={`hover:bg-gray-50 dark:hover:bg-gray-800 group ${isSubitem ? 'bg-white dark:bg-gray-800' : 'bg-white dark:bg-[#1e1e1e]'}`}
      >
        <td
          className={`p-0 w-12 border-r border-b border-gray-200 dark:border-gray-700 ${isSubitem ? 'bg-white dark:bg-gray-800' : 'bg-white dark:bg-[#1e1e1e] sticky left-0 z-10'}`}
        >
        ...
        {visibleColumns.map(column => {
          ...
          return (
            <td
              ...
              className={`p-0 h-9 border-r border-b border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100`}
            >
              <div
                onClick={e => {
  ...
  return (
    <div className='relative'>
      ...
      <div className='overflow-auto rounded-lg border border-gray-200 dark:border-gray-700 max-h-[calc(100vh-200px)] relative bg-white dark:bg-[#1e1e1e]'>
        <table className='min-w-full border-collapse'>
          <thead className='bg-white dark:bg-[#1e1e1e]'>{renderHeaderRow(true)}</thead>
          <tbody className='divide-y divide-gray-200 dark:divide-gray-700'>
            {groups?.map(group => (
              <React.Fragment key={group.groupId}>
                <tr className='bg-white dark:bg-[#1e1e1e] group'>
                  <td className='sticky left-0 z-10 bg-white dark:bg-[#1e1e1e] p-0 w-12 border-r border-gray-200 dark:border-gray-700'>
                    <IconButton
                      size='small'
                  ...
                  <React.Fragment key={item.taskId}>
                    ...
                    {expandedItemIds.includes(item.taskId) && (
                      <tr className='bg-gray-50/50 dark:bg-gray-900/50'>
                        <td colSpan={visibleColumns.length + 2} className='p-0 pl-[50px] py-2 relative'>
                          {/* Visual Connector: The "L" shape line */}
                          <div className='absolute left-[24px] top-[-10px] bottom-[20px] w-[26px] border-l-2 border-b-2 border-gray-300 dark:border-gray-600 rounded-bl-xl pointer-events-none z-0'></div>

                          {/* Subitem Card Container */}
                          <div className='bg-white dark:bg-gray-800 rounded-md shadow-sm border border-gray-200 dark:border-gray-700 mr-4 relative z-10 overflow-hidden'>
                            {/* Mini Header for Subitems */}
                            <div className='border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/50 px-3 py-1.5 flex items-center'>
                              <Typography
                                variant='caption'
                                  ...
                                  >
                                    {activeSubitemInput === item.taskId ? (
                                      ...
                                      <button
                                        ...
                                        className='flex items-center w-full text-left text-sm text-gray-400 dark:text-gray-500 hover:text-primary transition-colors py-1'
                                      >
                ...
                <tr className='bg-white dark:bg-[#1e1e1e] border-t border-gray-700 dark:border-gray-600'>
                  <td className='sticky left-0 z-10 bg-white dark:bg-[#1e1e1e] p-0 w-12 border-r border-gray-200 dark:border-gray-700'></td>
                  {visibleColumns.map(column => {
