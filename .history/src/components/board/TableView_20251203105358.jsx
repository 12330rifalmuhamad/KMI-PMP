...
import {
  ...
  FormControlLabel,
  Grid,
  useTheme // Hook tema untuk akses palet warna Vuexy
} from '@mui/material'
...
const PersonAvatar = ({ user }) => {
  if (!user) {
    return (
      <MuiAvatar
        sx={{ width: 26, height: 26, backgroundColor: 'action.selected', color: 'text.disabled' }}
        title='Unassigned'
      >
  ...
  return (
    <MuiAvatar sx={{ width: 26, height: 26, fontSize: '0.75rem', fontWeight: 600 }} title={user.userName}>
...
const StatusCell = ({ value, column }) => {
  const options = useMemo(() => {
    if (column.options?.length > 0) {
      return column.options.map(opt => ({
        ...
        color: opt.color,
        text:
          opt.color.includes('/10') || opt.color.includes('gray') ? 'text-gray-500 dark:text-gray-300' : 'text-white'
      }))
    }

    if (column.columnName.toLowerCase() === 'prioritas') {
      return [
        { label: 'Tinggi', color: 'bg-error-main/10', text: 'text-error-main' }, // Vuexy Error Color style
        { label: 'Medium', color: 'bg-warning-main/10', text: 'text-warning-main' }, // Vuexy Warning Color style
        { label: 'Rendah', color: 'bg-success-main/10', text: 'text-success-main' } // Vuexy Success Color style
      ]
    }

    return [
      { label: 'Sedang Dikerjakan', color: 'bg-warning-main', text: 'text-white' },
      { label: 'Buntu', color: 'bg-error-main', text: 'text-white' },
      { label: 'Selesai', color: 'bg-success-main', text: 'text-white' },
      { label: 'Belum Mulai', color: 'bg-secondary-main', text: 'text-white' }
    ]
  }, [column])
  ...
  if (column.columnName.toLowerCase() === 'prioritas') {
    // Styling a la Vuexy Badge
    const colors = option ? `${option.color} ${option.text}` : 'text-text-disabled border border-divider'

    // Note: Class names like bg-error-main/10 need Tailwind config to map to MUI palette,
    // or we assume generic Tailwind colors here for safety if exact Vuexy tailwind config isn't loaded.
    // Fallback to generic Tailwind for safety if not using mapped colors:
    const safeColors = option
      ? option.label === 'Tinggi'
    ...
    return (
      <div
        className={`flex items-center justify-center px-2 py-0.5 rounded-md text-[11px] font-semibold uppercase tracking-wide ${safeColors}`}
      >
        {displayText}
      </div>
    )
  ...
  // Mapping generic colors to safe Tailwind if MUI palette classes aren't available
  const displayClass =
    option?.label === 'Selesai'
      ? 'bg-green-500 text-white'
      : option?.label === 'Buntu'
        ? 'bg-red-500 text-white'
        : option?.label === 'Sedang Dikerjakan'
          ? 'bg-amber-500 text-white'
          : 'bg-gray-400 text-white'

  return (
    <div className={`flex items-center justify-center w-full h-full text-[11px] font-bold ${displayClass}`}>
      {displayText}
    </div>
  )
}
...
const DateCell = ({ value }) => {
  if (!value) return <span className='text-xs text-textSecondary'>—</span>
...
const FilesCell = ({ value }) => {
  ...
  return (
    <div className='flex items-center justify-center gap-1 text-textSecondary'>
      <i className='tabler-paperclip text-xs' />
      <span className='text-xs'>{fileCount > 0 ? fileCount : '-'}</span>
    </div>
  )
}

const LinkCell = ({ value }) => {
  if (!value) return <span className='text-textDisabled text-xs'>—</span>
  let href = value
  ...
  return (
    <a
      ...
      className='text-primary-main hover:text-primary-dark underline truncate text-xs'
...
const ProgressCell = ({ item, column }) => {
  ...
  return (
    <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', gap: 1 }}>
      <Box sx={{ width: '60%', height: 6, backgroundColor: 'action.hover', borderRadius: 5, overflow: 'hidden' }}>
        <Box
          sx={{
      ...
      <Typography
        variant='caption'
        sx={{ width: '40%', textAlign: 'right', fontSize: '0.7rem', color: 'text.secondary' }}
      >
        {progress.text}
      </Typography>
    </Box>
      ...
      <Box sx={{ width: 280, p: 1 }}>
        <List dense>
          ...
          {isUploading ? (
            ...
            [{ label: 'From Computer', icon: 'tabler-microphone', action: () => fileInputRef.current.click() }].map(
              item => (
...
export default function TableView({ board }) {
  const { mutate } = useSWRConfig()
  const theme = useTheme() // Access Vuexy theme
  ...
  const [activeSubitemInput, setActiveSubitemInput] = useState(null)
  ...
  const handleUpdateValue = async (itemToUpdate, columnToUpdate, newValue, isSubitem = false) => {
    ...
    if (!isSubitem) {
      ...
      setGroups(newBoardData.groups)
    }

    setEditingCell(null)
    setEditingTextValue(null)

    const endpoint = isSubitem
      ? `/api/subitems/${itemToUpdate.subitemId}/values`
      : `/api/tasks/${itemToUpdate.taskId}/values`
  ...
  const handleCreateSubitem = async parentTaskId => {
    ...
    await handleApiCall(
      fetch(`/api/tasks/${parentTaskId}/subitems`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: newSubitemTitle })
      })
    )
    setNewSubitemTitle('')
    setActiveSubitemInput(null)
  }

  const handleDeleteSubitem = async subitemId => {
    if (!confirm('Delete subitem?')) return
    await handleApiCall(fetch(`/api/subitems/${subitemId}`, { method: 'DELETE' }))
  ...
  const handleSaveTextValue = () => {
    ...
    const { item, column, currentValue, isSubitem } = editingTextValue

    handleUpdateValue(item, column, currentValue, isSubitem)
  }
  ...
  const handleCellClick = (event, item, column, isSubitem = false) => {
    if (column.columnName.toLowerCase() === 'item') {
      if (!isSubitem) setSelectedItem(item)
      ...
      setEditingTextValue({
        taskId: isSubitem ? item.subitemId : item.taskId,
      ...
      setEditingCell({ anchorEl: event.currentTarget, item, column, isSubitem })
    else if (column.columnType === 'FILES')
      setFilesPopover({ anchorEl: event.currentTarget, item: item, column: column, isSubitem })
  }
  ...
  const renderHeaderRow = (isMainHeader = false) => {
    return (
      <tr
        ...
        className={`bg-white dark:bg-[#2F3349] border-b border-gray-200 dark:border-gray-700 ${!isMainHeader ? 'bg-opacity-50' : ''}`}
      >
        <th
          className={`sticky left-0 top-0 z-30 bg-white dark:bg-[#2F3349] p-0 w-12 border-r border-gray-200 dark:border-gray-700`}
        >
        ...
        {visibleColumns?.map(column => (
          <th
            key={column.columnId}
            className={`p-0 text-left text-sm font-semibold text-gray-900 dark:text-gray-200 whitespace-nowrap border-r border-gray-200 dark:border-gray-700 sticky top-0 z-20 bg-white dark:bg-[#2F3349] ${isMainHeader ? 'table-column-draggable' : ''}`}
          >
            <div className='flex items-center justify-between px-3 py-3'>
              <span className='truncate flex items-center gap-2'>
                {isMainHeader && (
                  <i className='tabler-arrows-move col-handle text-gray-500 dark:text-gray-400 cursor-grab' />
                )}
                {column.columnName}
              </span>
        ...
        <th className='w-12 border-r border-gray-200 dark:border-gray-700 sticky top-0 z-20 bg-white dark:bg-[#2F3349]'>
          {isMainHeader && (
            <IconButton size='small' onClick={handleOpenAddColumnMenu}>
              <i className='tabler-plus text-gray-500 dark:text-gray-400' />
            </IconButton>
          )}
        </th>
      </tr>
  ...
  const RenderRow = ({ item, isSubitem = false }) => {
    return (
      <tr
        key={isSubitem ? item.subitemId : item.taskId}
        className={`hover:bg-gray-50 dark:hover:bg-[#3b405c] group ${isSubitem ? 'bg-white dark:bg-[#2F3349]' : 'bg-white dark:bg-[#2F3349]'}`}
      >
        <td
          className={`p-0 w-12 border-r border-b border-gray-200 dark:border-gray-700 ${isSubitem ? 'bg-white dark:bg-[#2F3349]' : 'bg-white dark:bg-[#2F3349] sticky left-0 z-10'}`}
        >
          <div className='flex justify-center items-center h-full'>
            <Checkbox className='group-hover:hidden' size='small' />
            <IconButton
              size='small'
              ...
              onClick={e =>
                setMenuAnchor({
                  ...
                  id: isSubitem ? item.subitemId : item.taskId
                })
              }
            >
              <i className='tabler-dots-vertical' />
        ...
        {visibleColumns.map(column => {
          const cellValue = (item.values || []).find(val => normalizeId(val.columnId) === normalizeId(column.columnId))
          let cellContent
          const isItemTitleCol = column.columnName.toLowerCase() === 'item'

          const isEditingTitle =
            editingTaskName?.taskId === (isSubitem ? item.subitemId : item.taskId) && isItemTitleCol

          const isEditingValue =
            editingTextValue?.taskId === (isSubitem ? item.subitemId : item.taskId) &&
            editingTextValue?.columnId === column.columnId
          ...
          switch (column.columnType) {
              ...
              cellContent = (
                <div className='flex justify-center'>
                  <Checkbox
                    ...
                    onChange={e => handleUpdateValue(item, column, e.target.checked.toString(), isSubitem)}
                    onClick={e => e.stopPropagation()}
                  />
                </div>
              )
            ...
            case 'LINK':
              ...
              break
            case 'NUMBER':
              cellContent = <span className='text-sm text-textPrimary'>{cellValue?.value || '—'}</span>
              break
            case 'PROGRESS':
                ...
                cellContent = (
                  <div className='flex items-center gap-2 pl-2'>
                    {!isSubitem && (
                      ...
                      >
                        <i
                          className={`tabler-chevron-right transition-transform text-gray-500 dark:text-gray-400 ${expandedItemIds.includes(item.taskId) ? 'rotate-90' : ''}`}
                        />
                      </IconButton>
                    )}
                    <span
                    ...
                    >
                      {isSubitem ? item.title || item.taskTitle : item.taskTitle}
                    </span>
                    {!isSubitem && !expandedItemIds.includes(item.taskId) && item.subitems?.length > 0 && (
                      <span className='ml-2 text-[10px] bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded-full border border-gray-300 dark:border-gray-600'>
          ...
          return (
            <td
              key={`${isSubitem ? item.subitemId : item.taskId}-${column.columnId}`}
              className={`p-0 h-10 border-r border-b border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100`}
            >
              <div
                onClick={e => {
                  if (isEditingTitle || isEditingValue) return
                  handleCellClick(e, item, column, isSubitem)
                }}
              ...
              >
                {isEditingTitle ? (
                  ...
                  <TextField
                    ...
                    InputProps={{ disableUnderline: true, className: '!text-gray-900 dark:!text-gray-100' }}
                    onClick={e => e.stopPropagation()}
                  />
                ) : isItemTitleCol ? (
                  <div
        ...
        <td className='border-r border-gray-200 dark:border-gray-700 border-b'></td>
      </tr>
    )
  }

  return (
    <Card sx={{ boxShadow: 'none' }} className='border border-gray-200 dark:border-gray-700'>
      <div className='relative'>
        ...
        <div className='overflow-auto rounded-none border-t border-gray-200 dark:border-gray-700 max-h-[calc(100vh-200px)] relative bg-white dark:bg-[#2F3349]'>
          <table className='min-w-full border-collapse'>
            <thead className='bg-white dark:bg-[#2F3349]'>{renderHeaderRow(true)}</thead>
            <tbody className='divide-y divide-gray-200 dark:divide-gray-700'>
              {groups?.map(group => (
                <React.Fragment key={group.groupId}>
                  <tr className='bg-white dark:bg-[#2F3349] group'>
                    <td className='sticky left-0 z-10 bg-white dark:bg-[#2F3349] p-0 w-12 border-r border-gray-200 dark:border-gray-700'>
                      <IconButton
                        size='small'
                    ...
                    <td
                      ...
                      className='p-2 font-bold text-gray-900 dark:text-gray-100'
                  ...
                  {group.items?.map(item => (
                    <React.Fragment key={item.taskId}>
                      {/* PARENT ROW */}
                      <RenderRow item={item} isSubitem={false} />

                      {/* SUBITEMS CONTAINER ROW */}
                      {expandedItemIds.includes(item.taskId) && (
                        <tr className='bg-gray-50/50 dark:bg-[#25293C]'>
                          <td colSpan={visibleColumns.length + 2} className='p-0 pl-[50px] py-3 relative'>
                            {/* Visual Connector: The "L" shape line */}
                            <div className='absolute left-[24px] top-[-10px] bottom-[25px] w-[26px] border-l-2 border-b-2 border-gray-300 dark:border-gray-600 rounded-bl-xl pointer-events-none z-0'></div>

                            {/* Subitem Card Container - Nested Card Look */}
                            <div className='bg-white dark:bg-[#2F3349] rounded-md shadow-sm border border-gray-200 dark:border-gray-700 mr-4 relative z-10 overflow-hidden'>
                              {/* Mini Header for Subitems */}
                              <div className='border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-[#25293C] px-3 py-1.5 flex items-center'>
                                <Typography
                                  variant='caption'
                                  className='text-gray-500 dark:text-gray-400 font-semibold uppercase tracking-wider'
                                >
                                  Subitem
                                </Typography>
                              </div>
                              ...
                              <table className='w-full'>
                                <tbody>
                                  {item.subitems?.map(subitem => (
                                    <RenderRow key={subitem.subitemId} item={subitem} isSubitem={true} />
                                  ))}
                                  {/* Input New Subitem Row inside Card */}
                                  <tr className='bg-white dark:bg-[#2F3349] hover:bg-gray-50 dark:hover:bg-[#3b405c]'>
                                    <td className='p-0 w-12 border-r border-gray-200 dark:border-gray-700'></td>
                                    <td
                                      className='p-2 text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700'
                                      colSpan={visibleColumns.length}
                                    >
                                      {activeSubitemInput === item.taskId ? (
                                        <TextField
                                          fullWidth
                                          autoFocus
                                          variant='standard'
                                          ...
                                          onBlur={() => handleCreateSubitem(item.taskId)}
                                          onKeyDown={e => e.key === 'Enter' && handleCreateSubitem(item.taskId)}
                                        ...
                                        <button
                                          onClick={() => setActiveSubitemInput(item.taskId)}
                                          className='flex items-center w-full text-left text-sm text-gray-400 dark:text-gray-500 hover:text-primary-main transition-colors py-1'
                                        >
                                          <i className='tabler-plus mr-2 text-xs' />
                                          Add subitem
                                        </button>
                                      )}
                                    </td>
                  ...
                  <tr className='bg-white dark:bg-[#2F3349]'>
                    <td className='sticky left-0 z-10 bg-white dark:bg-[#2F3349] p-0 w-12 border-r border-gray-200 dark:border-gray-700'>
                      <div className='flex justify-center'>
                        <Checkbox className='opacity-0' />
                      </div>
                    </td>
                    <td className='p-2 text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700'>
                  ...
                  <tr className='bg-white dark:bg-[#2F3349] border-t border-gray-700 dark:border-gray-600'>
                    <td className='sticky left-0 z-10 bg-white dark:bg-[#2F3349] p-0 w-12 border-r border-gray-200 dark:border-gray-700'></td>
                    {visibleColumns.map(column => {
                      ...
                      if (column.columnName.toLowerCase() === 'item')
                        summaryContent = (
                          <Typography variant='caption' className='text-gray-400 dark:text-gray-500 px-3'>
                        ...
                        summaryContent = (
                          <div className='flex h-4 rounded-sm overflow-hidden mx-2'>
                            {Object.entries(summary).map(([status, count]) => {
                              ...
                              if (status === 'Selesai') bgColor = 'bg-success-main'
                              else if (status === 'Sedang Dikerjakan') bgColor = 'bg-warning-main'
                              else if (status === 'Buntu') bgColor = 'bg-error-main'
                        ...
                        summaryContent = (
                          <Button
                            ...
                            className='!text-gray-500 dark:!text-gray-400 !normal-case !font-semibold !p-0 !min-w-0'
                          >
                        ...
                        summaryContent = (
                          <Typography variant='caption' className='text-gray-500 dark:text-gray-400 px-3'>
                            -
                          </Typography>
                        )
                      }
        ...
        >
          <MenuItem
            onClick={() => {
              if (menuAnchor.type === 'subitem') handleDeleteSubitem(menuAnchor.id)
              else if (menuAnchor.type === 'task') handleDeleteTask(menuAnchor.id)
              else handleDeleteGroup(menuAnchor.id)
            }}
            sx={{ color: 'error.main' }}
          >
            Delete
        ...
        {editingCell && (
          <ValueEditorPopover
            ...
            onValueSelect={newValue =>
              handleUpdateValue(editingCell.item, editingCell.column, newValue, editingCell.isSubitem)
            }
          />
        )}
        <FilesPopover
          ...
          onFileUploaded={newValue => {
            handleUpdateValue(filesPopover.item, filesPopover.column, newValue, filesPopover.isSubitem)
          }}
        />
        <Menu
