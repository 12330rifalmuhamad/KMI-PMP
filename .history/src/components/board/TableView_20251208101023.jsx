...
// Helper: Format Tampilan Tanggal Timeline (Contoh: Dec 31, '24)
...
const TimelineCell = ({ value, column }) => {
  ...
  // Warna background timeline (Hijau Timeline)
...
const BoardCell = ({ item, column, board, onUpdateValue, onClick }) => {
  ...
  switch (column.columnType) {
    ...
    case 'TIMELINE': // --- CASE BARU UNTUK TIMELINE ---
      return (
...
// =================================================================
// SUBITEMS VIEW (Updated with Delete)
// =================================================================
const SubItemsView = ({
  parentItem,
  board,
  ...
  onCreateSubitem,
  onCellClick,
  onDeleteSubitem // Prop baru untuk handle delete
}) => {
                    ...
                    <button
                      ...
                      onClick={e => {
                        ...
                        if (confirm('Are you sure you want to delete this subitem?')) {
                          onDeleteSubitem(parentItem.taskId, sub.taskId)
                        }
                      }}
                    >
...
const TimelinePopover = ({ anchorEl, onClose, item, column, onSave }) => {
  ...
  // Hitung Durasi (End - Start + 1 hari)
  const duration = useMemo(() => {
    ...
    // Jika start == end, itu 1 hari kerja, tapi milestone sering dianggap 0 durasi atau 1 hari moment
    // Di monday.com biasanya inklusif (+1)
    return diffDays + 1
  }, [startDate, endDate])
  ...
  const handleSave = () => {
    ...
    if (isMilestone) {
      finalEnd = finalStart // Milestone: Start = End
    } else {
      // Validasi agar Start <= End
    ...
    // Simpan format "YYYY-MM-DD,YYYY-MM-DD"
...
// =================================================================
// EDITOR & MODAL COMPONENTS (EXISTING)
// =================================================================
        ...
        <CardContent className='flex flex-col gap-4'>
          ...
          <FormControl fullWidth>
            ...
            <Select value={columnType} label='Column Type' onChange={e => setColumnType(e.target.value)}>
              ...
              <MenuItem value={'TIMELINE'}>Timeline</MenuItem> {/* Added Timeline */}
...
export default function TableView({ board }) {
  ...
  // --- NEW: HANDLE DELETE SUBITEM ---
  const handleDeleteSubitem = async (parentTaskId, subitemId) => {
    // 1. Optimistic Update (Hapus dari state lokal)
      ...
      prevGroups.map(group => ({
        ...
        items: group.items.map(item => {
          if (item.taskId === parentTaskId) {
            return {
              ...item,
              subItems: item.subItems ? item.subItems.filter(sub => sub.taskId !== subitemId) : []
            }
          }
    ...
    try {
      ...
      alert('Gagal menghapus subitem')
      mutate(`/api/boards/${board.boardId}`)
    }
  }

  // --- GENERAL UPDATE HANDLER (Parent & Subitem) ---
  ...
  const handleUpdateValue = async (itemToUpdate, columnToUpdate, newValue) => {
    handleGeneralUpdate(itemToUpdate, columnToUpdate, newValue)
  }
  ...
  const handleCellClick = (event, item, column) => {
    if (column.columnName.toLowerCase() === 'item') setSelectedItem(item)
      ...
      // TRIGGER TIMELINE POPOVER DISINI
